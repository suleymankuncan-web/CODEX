...
// Trace: FR-DIAG-08, FR-DIAG-09, FR-DIAG-11; NFR-01, NFR-02; AC-01.
describe("staging remediation diagnostic CLI boundary", () => {
  it.each([
    {
...
function runCli(overrides: Partial<Record<string, string>>) {
  ...
  for (const name of [
    ...
    "DB_SSL_CA",
    "DB_SSL_MODE",
    "STAGING_REMEDIATION_DIAGNOSTIC_REVIEWED_COMMIT",
  ]) {
    delete env[name];
  }
  env.NODE_ENV = "test";
  ...
  return spawnSync(
    process.execPath,
    ["-r", "ts-node/register", join(process.cwd(), "scripts", "staging-remediation-diagnostic.ts")],
