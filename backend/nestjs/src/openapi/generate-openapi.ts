...
import { applyWorkforceOpenApi } from "./workforce-openapi"; import { applyPersonnelCorrectionOpenApi } from "./personnel-correction-openapi"; import { applyRegionManagerDirectoryOpenApi } from "./region-manager-directory-openapi";
...
const authUserAccountSchema = {
  ...
  properties: {
    ...
    identityOperation: { type: "string", nullable: true },
    identityStatus: { type: "string", nullable: true },
    identityErrorCode: { type: "string", nullable: true },
  },
};
...
const targetDistributionRequestSchema = {
  type: "object",
  required: [
    "requestId", "companyId", "regionId", "storeId", "storeName", "requestMonth", "targetLabel",
    "totalTargetValue", "allocationCount", "status", "requestReason", "allocations",
    "submittedByUserId", "approvedByUserId", "approvedAt", "approvalNote", "approvalMode",
    "originalTotalTargetValue", "approvedTotalTargetValue", "originalAllocations",
    "approvedAllocations", "createdAt", "updatedAt",
  ],
  properties: {
    ...
    allocations: { type: "array", items: targetDistributionAllocationSchema },
    ...
    approvalMode: { type: "string", enum: ["direct", "adjusted"], nullable: true },
    originalTotalTargetValue: { type: "number", nullable: true },
    approvedTotalTargetValue: { type: "number", nullable: true },
    originalAllocations: { type: "array", items: targetDistributionAllocationSchema },
    approvedAllocations: { type: "array", items: targetDistributionAllocationSchema },
    createdAt: { type: "string" },
    updatedAt: { type: "string" },
  },
};
...
const workforceSellerCodeRequestSchema = {
  type: "object",
  required: [
    "requestId", "companyId", "regionId", "storeId", "storeCode", "storeName",
    "storeType", "requestType", "status", "firstName", "lastName", "nationalIdLast4",
    "phoneNumber", "username", "email", "hireDate", "requestedPositionId", "positionCode",
    "positionName", "employmentType", "requestedSellerCode", "approvedSellerCode",
    "lastReferenceSellerCode", "submittedByUserId", "reviewedByUserId", "reviewedAt",
    "reviewNote", "employeeId", "createdAt", "updatedAt",
  ],
  properties: {
    requestId: { type: "string" },
    companyId: { type: "string" },
...
async function generateOpenApi(): Promise<void> {
  ...
  applyStoreActionPlanOpenApi(document); applyChecklistCommandOpenApi(document); applySalesTargetIncentiveWorkspaceOpenApi(document); applyTargetWorkspaceOpenApi(document); applyWorkforceWorkspaceOpenApi(document);
  ...
  preserveOpenApiBaselineFromFile(document, outputPath); applyTaskCommandWorkspaceOpenApi(document); applyVmReferenceManagementOpenApi(document); applyPersonnelCorrectionOpenApi(document); applyRegionManagerDirectoryOpenApi(document);
