...
} from "./photo-media-asset-restore.repository";
...
export class PhotoMediaAssetRepository implements PhotoMediaAssetRepositoryPort {
  constructor(
    ...
    private readonly storageIdentity: PhotoMediaStorageIdentity =
      HISTORICAL_R2_PHOTO_MEDIA_STORAGE_IDENTITY,
  ) {}
  ...
  async claimRestoreCandidate(input: Record<string, unknown>) {
    return claimPhotoMediaRestoreCandidate(this.databaseService, this.storageIdentity, input);
  }

  async reserveRestoreGeneration(input: Record<string, unknown>): Promise<void> {
    return reservePhotoMediaRestoreGeneration(this.databaseService, this.storageIdentity, input);
  }

  async markRestoreVerified(input: Record<string, unknown>): Promise<void> {
    return markPhotoMediaRestoreVerified(this.databaseService, this.storageIdentity, input);
  }
