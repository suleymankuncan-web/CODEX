...
export function rankingDailyComponentsSql(scope: "store" | "employee", guard?: string) {
  ...
  return `WITH daily AS (
    SELECT ${employee} ka.store_id, ka.period_start AS day,
      ...
      COALESCE(MAX(ka.actual_value) FILTER (WHERE kd.kpi_code = 'NET_SALES'),
        MAX(ka.actual_value) FILTER (WHERE kd.kpi_code = 'TARGET_ACHIEVEMENT'),
        MAX(ka.actual_value) FILTER (WHERE kd.kpi_code = 'STORE_SALES'),
        MAX(ka.actual_value) FILTER (WHERE kd.kpi_code = 'SALES_TARGET_ACHIEVEMENT')) AS achievement,
