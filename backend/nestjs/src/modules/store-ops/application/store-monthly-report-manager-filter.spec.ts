...
describe('monthly report manager selection', () => {
  function createService() {
    ...
    const directory = { listCompanyRegionManagerDirectory: jest.fn(async () => [
      { id: 'manager-1', label: 'Aynı Ad', storeIds: ['store-1'] },
  ...
  it('uses the selected user ID and intersects its direct stores with the original scope', async () => {
    ...
    expect(directory.listCompanyRegionManagerDirectory).toHaveBeenCalledWith({ companyIds: ['company-1'] });
    expect(report.getStoreMonthlyReportPackageRows).toHaveBeenCalledWith(expect.objectContaining({ companyIds: ['company-1'], selectedStoreIds: ['store-1'] }));
  });
  ...
  it('never calls the unbounded directory when company scope is empty', async () => {
    ...
    expect(directory.listCompanyRegionManagerDirectory).not.toHaveBeenCalled();
    expect(report.getStoreMonthlyReportPackageRows).toHaveBeenCalledWith(expect.objectContaining({ selectedStoreIds: [] }));
  });
