...
function setup() {
  const repository = { listRegionManagerDirectory: jest.fn().mockResolvedValue([
...
describe("RegionManagerDirectoryService", () => {
  it("lists only managers with direct store assignments in the viewer company", async () => {
    ...
    expect(repository.listRegionManagerDirectory).toHaveBeenCalledWith({ companyIds: ["company-a"] });
  });
  it.each(["REGION_MANAGER", "STORE_MANAGER", "STORE_PERSONNEL"])("denies %s", async role => {
    ...
    expect(repository.listRegionManagerDirectory).not.toHaveBeenCalled();
  });
