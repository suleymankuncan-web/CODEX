  ...
  it("keeps a store manager ledger on assigned stores and maps the read envelope", async () => {
    const requestCenterReadRepository = {
      listRequests: jest.fn(async () => ({
        ...
        total: 31,
        summary: { open: 20, done: 11, returned: 2, overdue: 4, periods: ["2026-07"] },
        limit: 15,
        offset: 15,
      })),
    };
    ...
    expect(result).toEqual({
      items: [
        expect.objectContaining({
          ...
          regionName: "Marmara",
          regionManagerNames: ["Region Manager", "Second Manager"],
          status: "pending_region_approval",
          createdAt: "2026-07-08T09:00:00.000Z",
          waitingSince: "2026-07-08T09:00:00.000Z",
          nextOwner: "region",
  ...
  it("uses only the manager's active store assignments, not the legacy region scope", async () => {
    const requestCenterReadRepository = {
      listRequests: jest.fn(async () => ({
        items: [],
        total: 0,
        summary: { open: 0, done: 0, returned: 0, overdue: 0, periods: [] },
        limit: 15,
        offset: 0,
      })),
    };
