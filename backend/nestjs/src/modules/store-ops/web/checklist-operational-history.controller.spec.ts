...
describe("ChecklistOperationalHistoryController", () => {
  it("keeps the endpoint read-only and limited to the three approved personas", async () => {
    ...
    await expect(controller.read(
      { user: { roleCodes: ["REPORT_VIEWER"], roleScopes: { REPORT_VIEWER: { companyIds: ["company-a"], regionIds: [], storeIds: [] } } } } as never,
    ...
    expect(service.read).toHaveBeenCalledWith(expect.objectContaining({
      actorRoleCodes: ["REPORT_VIEWER"], storeId: "11111111-1111-4111-8111-111111111111", range: "3m",
    }));
  });
});
