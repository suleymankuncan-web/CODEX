...
export class IncentiveFinalApprovalController {
  ...
  async approve(@Req() request: { user: AuthenticatedUser }, @Body() body: ApproveFinalIncentivePackageDto) {
    return this.service.approveFinalPackage({ actor: request.user, periodKey: body.period, regionId: body.regionId, regionPackageId: body.regionPackageId, submittedAt: body.submittedAt });
  }
}
