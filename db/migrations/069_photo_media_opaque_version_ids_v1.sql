...
ALTER TABLE ops.media_asset
    ADD COLUMN IF NOT EXISTS raw_object_version_id TEXT,
    ADD COLUMN IF NOT EXISTS thumbnail_object_version_id TEXT;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'ck_media_asset_raw_object_version_id_private'
          AND conrelid = 'ops.media_asset'::regclass
    ) THEN
        ALTER TABLE ops.media_asset
            ADD CONSTRAINT ck_media_asset_raw_object_version_id_private CHECK (
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'ck_media_asset_thumbnail_object_version_id_private'
          AND conrelid = 'ops.media_asset'::regclass
    ) THEN
        ALTER TABLE ops.media_asset
            ADD CONSTRAINT ck_media_asset_thumbnail_object_version_id_private CHECK (
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'ck_media_asset_replica_object_version_id_private'
          AND conrelid = 'ops.media_asset_replica'::regclass
    ) THEN
        ALTER TABLE ops.media_asset_replica
            ADD CONSTRAINT ck_media_asset_replica_object_version_id_private CHECK (
