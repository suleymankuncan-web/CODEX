...
Next:
1. Complete the owner-approved Command Canvas cutover in `docs/plans/checklist-command-canvas-exact-production-cutover-plan-v2.md`. P1A, P2, P2B, P3, P4, P1B, P5 and P6 are merged through PR #996 (`8a9b74b9`). PR #997 is the final P7 cutover: it resolves the VM/Super Admin Command surfaces, removes the legacy checklist DOM/chunk and orphan CSS/tests, migrates remaining deep-link producers, and carries green automated parity and canonical release evidence. On 15 July 2026 the product owner explicitly waived physical iOS Safari/Android Chrome evidence as a merge gate and accepted that residual risk for controlled-pilot observation; the unperformed checks must not be reported as PASS. P7 now requires only final-head checks, squash merge, merged-SHA verification and controlled post-merge deployed smoke.
