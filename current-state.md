...
Next:
1. Continue the owner-approved Command Canvas cutover in `docs/plans/checklist-command-canvas-exact-production-cutover-plan-v2.md`. P1A, P2, P2B, P3, P4, P1B and P5 are merged through PR #995. P6 now replaces the Store Manager checklist route with an explicit authorized-store surface: every managed store is shown, checklist/result/acknowledgement flows stay in the canonical overlay, Living Store Record remains store-scoped, and region/company aggregates plus weekly-plan mutation remain unavailable. After P6 merges, only P7 remains: resolve the VM visual contract, remove the legacy checklist DOM/chunk atomically, migrate the remaining deep-link producers, and complete all-role/real-device closeout.
