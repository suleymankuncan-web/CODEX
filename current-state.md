...
No-Go without repair or mutation. PR #952 merged REM-1A; PR #953 merged one owner-confirmed REM-1B run with stable 70 family hits, 72 source records, nine buckets, and unresolved distinct-person count. REM-2 locked TARGET, organization, and assignment semantics including rotation-based winner authority; restore and concurrency decisions remain unset. DG1-C retirement remains usage-gated.
...
- Merged REM-1B evidence explains the stable 70 hits without mutation. `codex/rem-2-owner-decision-options` locks TARGET and organization lifecycles plus assignment rules: support is secondary, exactly one primary is open, dates do not overlap, and approved effective-dated rotation determines the primary winner. The two row-level winners and all affected classifications still require secure lifecycle evidence. Separate history-preserving writers are required. Restore and concurrency remain UNSET; DB-CONSTRAINTS remains No-Go and no second run, DML, DDL, paid service, runtime change, or production operation is authorized.
