...
Now:
- The owner-approved photo-evidence train is active. PR #1020 locked the product/provider gates, PR #1021 merged the provider-neutral schema foundation, PR #1022 merged disabled-by-default synthetic private-media storage, and PR #1023 merged checklist item evidence at `2d7047089a2acc6004aeb6ca1140c80fcba792c5`. Exact-head authenticated synthetic R2 proof, physical iPhone Safari and Android Chrome owner attestations, Release Rehearsal, and Post-Merge Verification are green. PR-5 now runs on `codex/store-action-photo-review-v2`: V1/V2 lifecycle pinning, plan/actor-bound synthetic solution upload, immutable submit/review attempts, CAS/idempotency/row-lock guards, assigned Region Manager approval/rejection, and the Command Canvas review drawer are implemented and locally verified but remain default-disabled pending staging activation. Normal rollback disables new V2 submission while leaving Region Manager review enabled to drain the explicit pending queue; destructive rollback refuses used V2 data. Exactly one byte-identical fixture remains admitted and rechecked by SHA-256; this is `fixture_identity_only`, not malware scanning. No Render ClamAV service is authorized, camera and arbitrary gallery media remain disabled, and real photographs remain No-Go until a separately approved and proven malware-scanner adapter exists. The R2 posture remains two distinct private Standard EU buckets, separate bucket-scoped credentials, verified recovery before `ready`, 8 GiB / 750k Class A / 7.5m Class B ceilings, 30-day `locked/` safety window, RPO 0 and manual RTO <=24h. Runtime controls remain disabled by default and no real-photo, paid activation or broad-production authority exists. See `docs/plans/checklist-photo-media-storage-provider-decision-v1.md`, `docs/plans/checklist-item-evidence-pr4-execution-spec-v1.md`, and `docs/plans/store-action-photo-review-pr5-execution-spec-v1.md`.
