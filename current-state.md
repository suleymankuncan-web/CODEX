...
- PR #965 merged the exact-SHA REM-8 evidence at `3ab884f6ed922f5339d6a8890d7d4d69590710b8`. TARGET DB-C5 is the sole active constraint slice. Its repository implementation owns migration 060, exact provenance/checksum portability, fresh read-only preflight, canonical migration apply, strict postflight/receipt validation, one-shot evidence, PostgreSQL 17 semantics, and rollback; it performs no staging DDL. Target-reference supersession remains a separate unimplemented application-history gap and is not claimed by DB-C5.
...
Next:
1. Complete and merge the TARGET-only DB-C5 repository implementation PR; no staging access occurs in that PR.
2. Create the evidence branch from the exact merged SHA, run the one-shot staging launcher once under the locked 5s/30s window, and merge only sanitized evidence/handoff files.
...
4. Keep target-reference supersession as a separately scoped application-history gap; do not mix it into DB-C5.
