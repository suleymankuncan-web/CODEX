...
Next:
1. Continue the owner-approved Command Canvas cutover in `docs/plans/checklist-command-canvas-exact-production-cutover-plan-v2.md`: P1A merged as PR #988 and P2 merged as PR #989. Fresh P3 inspection proved that command-row capabilities cannot identify a stable Region Manager planning region, so the isolated read-only P2B contract supplies authenticated, server-searched and mechanically paged named region options before P3 resumes. P1B remains preserved from merged P1A and must be rebased only after P3; neither its intentional changes nor the preserved P3 planner may be reset or merged as partial evidence.
