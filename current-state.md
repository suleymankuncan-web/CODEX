...
Now:
- The owner-approved photo-evidence train is active. PR #1020 locked the product/provider gates, PR #1021 merged the provider-neutral schema foundation, and PR #1022 merged disabled-by-default synthetic private-media storage. PR-4 local work is on `codex/checklist-item-evidence-v1`: item policy snapshots, exact evidence links, both completion guards, checklist-scoped authenticated reads, and a server-digest-allowlisted synthetic picker/retry/preview/remove path now exist behind disabled-by-default controls. On 2026-07-27 the owner locked a PR-4-only zero-cost safety revision: exactly one byte-identical fixture is admitted and rechecked by SHA-256; this is `fixture_identity_only`, not malware scanning. No Render ClamAV service is authorized, camera and arbitrary gallery media remain disabled, and real photographs remain No-Go until a separately approved and proven malware-scanner adapter exists. Prior canonical proof belongs to the old head and must rerun after this revision. PR-4 remains merge `No-Go` until the revised exact-head authenticated synthetic R2 receipt and representative physical iOS Safari/Android Chrome picker/retry/preview receipts exist. The R2 posture remains two distinct private Standard EU buckets, separate bucket-scoped credentials, verified recovery before `ready`, 8 GiB / 750k Class A / 7.5m Class B ceilings, 30-day `locked/` safety window, RPO 0 and manual RTO <=24h. Runtime controls remain disabled by default and no real-photo, paid activation or broad-production authority exists. See `docs/plans/checklist-photo-media-storage-provider-decision-v1.md` and `docs/plans/checklist-item-evidence-pr4-execution-spec-v1.md`.
