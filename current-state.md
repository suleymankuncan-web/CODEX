...
Next:
1. Continue the owner-approved Command Canvas cutover in `docs/plans/checklist-command-canvas-exact-production-cutover-plan-v2.md`. P1A, P2, P2B, P3, P4, P1B, P5 and P6 are merged through PR #996 (`8a9b74b9`). Store Manager now has an explicit authorized-store surface: every managed store is shown, checklist/result/acknowledgement flows stay in the canonical overlay, Living Store Record remains store-scoped, and region/company aggregates plus weekly-plan mutation remain unavailable. Only P7 remains: its working diff resolves the VM/Super Admin Command surfaces, removes the legacy checklist DOM/chunk and orphan CSS/tests, and migrates remaining deep-link producers. P7 still requires final canonical verification, PR merge, post-merge smoke and recorded physical iOS Safari/Android Chrome evidence before the plan can be called complete.
