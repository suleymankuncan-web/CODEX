...
No-Go without repair or mutation. PR #952 merged REM-1A; PR #953 merged one owner-confirmed REM-1B run with stable 70 family hits, 72 source records, nine buckets, and unresolved distinct-person count. REM-2 locked TARGET, organization, and primary-versus-support assignment semantics; winner, restore, and concurrency decisions remain unset. DG1-C retirement remains usage-gated.
...
- Merged REM-1B evidence explains the stable 70 hits without mutation. `codex/rem-2-owner-decision-options` locks TARGET and organization lifecycles plus assignment rules: support may overlap as secondary, exactly one primary may be open, end dates are inclusive, and the next primary starts the following day. The two primary-overlap winners and all affected row classifications still require lifecycle evidence. Separate history-preserving writers are required. Other owner fields are UNSET; DB-CONSTRAINTS remains No-Go and no second run, DML, DDL, paid service, runtime change, or production operation is authorized.
