...
- The first post-#955 evidence attempt stopped before receipt creation because an untracked helper passed the CA path instead of PEM content. PR #956 then merged the versioned launcher, but its one-shot attempt also stopped before any DB connection because Windows rejected direct `spawnSync('npm.cmd')` with `EINVAL`. Neither consumed SHA is retried; neither produced a V2 result or mutation. The next implementation uses the repository's reviewed `cmd.exe /d /s /c` Windows wrapping while retaining CA/project-ref/branch/SHA/one-shot/receipt guards.
...
Next:
1. Merge the tested Windows launcher invocation fix through normal checks, sync `main`, then create a fresh evidence-only branch pinned to that exact merged SHA. Both earlier failed attempt markers remain preserved and are never deleted to manufacture a retry.
2. Resolve the standing-authorized technical inputs without exposing them, bind the exact staging identity and a bounded Europe/Istanbul run window, and execute the newly merged SHA once. A launcher/query/runner change requires another implementation PR rather than an evidence-branch patch.
