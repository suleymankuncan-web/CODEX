...
Bu planin ilk adimi uygulandi:

- `scripts/test-suite-hygiene-contract.test.mjs` icine master-data bootstrap service 29-test guard eklendi.
- Guard, mevcut `master-data-bootstrap.service.spec.ts` icindeki 29 test adinin tam 1 kez kaldigini kontrol eder.
- Henuz test tasima/split yapilmadi.

Siradaki mantikli adim: Onay verilirse yalnizca Bucket A staging/normalization testlerini `master-data-bootstrap-staging.service.spec.ts` dosyasina mekanik olarak tasimak. Validation ve promotion safety tarafina bu adimda dokunmamak.
