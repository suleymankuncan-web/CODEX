...
## Son Import Batch Source Test Split V1

30 Nisan 2026 itibariyla Test Suite Hygiene V1 ikinci guvenli dilimi uygulandi.
...
- Integration source CRUD, lookup, audit, duplicate-source, inactive-source ve deactivate-guard testleri `integration-sources.e2e-spec.ts` dosyasina tasindi.
- `import-batch.e2e-spec.ts` staging, reconciliation, mapping, retry ve import batch evidence testlerini tasimaya devam eder.
- Toplam 32 import/integration e2e test adi korunur.
- Production code degismedi.

Kapanis notu:

- Bu slice gosterisli degil ama bakim riskini dusurur.
- `import-batch.e2e-spec.ts` 2355 satira, yeni `integration-sources.e2e-spec.ts` 656 satira indi.
- Guard iki dosyada toplam 32 testin ve integration source test adlarinin tam 1 kez kaldigini kontrol eder.
...
## Son Competition Repository Test Split V1

30 Nisan 2026 itibariyla Test Suite Hygiene V1 dorduncu guvenli dilimi uygulandi.
...
- Team template repository list/create/deactivate/update/clone testleri `competition-team-template.repository.spec.ts` dosyasina tasindi.
- `competition.repository.spec.ts` stage, package plan, finalization, score ve scope repository testlerini tasimaya devam eder.
- Toplam 27 competition repository test adi korunur.
- Production code degismedi.

Kapanis notu:

- Master-data bu pass'te bilerek bolunmedi; is kurali yogunlugu daha yuksek.
- `competition.repository.spec.ts` 1362 satira indi.
- `competition-team-template.repository.spec.ts` 403 satir.
- Guard iki dosyada toplam 27 testin ve team template test adlarinin tam 1 kez kaldigini kontrol eder.

Siradaki mantikli adim: kalan buyuk backend spec adaylarini tekrar tartmak; ilk bakilacak dusuk risk adaylari `competition.service.spec.ts`, `snapshot-run.e2e-spec.ts` ve `auth-scope.e2e-spec.ts`.

## Son Competition Stage Package Plan Test Split V1

30 Nisan 2026 itibariyla Test Suite Hygiene V1 besinci guvenli dilimi uygulandi.
...
- Stage package plan draft/review/clone/audit testleri `competition-stage-package-plan.repository.spec.ts` dosyasina tasindi.
- `competition.repository.spec.ts` stage creation, execution, finalization, score ve scope repository testlerini tasimaya devam eder.
- `competition-team-template.repository.spec.ts` team template testlerini tasimaya devam eder.
- Toplam 27 competition repository test adi korunur.
- Production code degismedi.

Kapanis notu:

- `competition.repository.spec.ts` 634 satira indi.
- `competition-stage-package-plan.repository.spec.ts` 832 satir.
- `competition-team-template.repository.spec.ts` 403 satir.
- Guard uc dosyada toplam 27 testin, stage package plan test adlarinin ve team template test adlarinin tam 1 kez kaldigini kontrol eder.
...
## Son Competition Service Team Template Test Split V1

30 Nisan 2026 itibariyla Test Suite Hygiene V1 yedinci guvenli dilimi uygulandi.
...
- Team-template list/create/update/deactivate/clone service testleri `competition-team-template.service.spec.ts` dosyasina tasindi.
- `competition.service.spec.ts` stage, package, plan, finalization ve scoped detail service testlerini tasimaya devam eder.
- Toplam 24 competition service test adi korunur.
- Production code degismedi.

Kapanis notu:

- `competition.service.spec.ts` 829 satira indi.
- `competition-team-template.service.spec.ts` 282 satir.
- Guard iki dosyada toplam 24 testin, team-template service test adlarinin ve core service test adlarinin tam 1 kez kaldigini kontrol eder.

Siradaki mantikli adim: `auth-scope.e2e-spec.ts` dosyasini riskli guvenlik matrisi olarak once analiz etmek; mekanik boundary netse bolmek, degilse dokunmamak. Master-data tarafina hala ayri plan olmadan girilmez.

## Son Auth Action Scope Test Split V1

30 Nisan 2026 itibariyla Test Suite Hygiene V1 sekizinci guvenli dilimi uygulandi.
...
- Target-distribution ve checklist action-scope authorization testleri `auth-action-scope.e2e-spec.ts` dosyasina tasindi.
- `auth-scope.e2e-spec.ts` read-scope, role, session ve JWT testlerini tasimaya devam eder.
- Toplam 18 auth scope integration test adi korunur.
- Production code degismedi.

Kapanis notu:

- `auth-scope.e2e-spec.ts` 581 satira indi.
- `auth-action-scope.e2e-spec.ts` 438 satir.
- Guard iki dosyada toplam 18 testin, read/session test adlarinin ve action-scope test adlarinin tam 1 kez kaldigini kontrol eder.
...
Bu planin guvenli adimlari uygulandi:

- `scripts/test-suite-hygiene-contract.test.mjs` icine master-data bootstrap service 29-test guard eklendi.
- Guard, master-data bootstrap service test adlarinin aktif dosya setinde tam 1 kez kaldigini kontrol eder.
- Bucket A staging/normalization testleri `master-data-bootstrap-staging.service.spec.ts` dosyasina mekanik olarak tasindi.
- Bucket C read-model/readiness testleri `master-data-bootstrap-read-models.service.spec.ts` dosyasina mekanik olarak tasindi.
- `master-data-bootstrap.service.spec.ts` validation ve promotion safety testlerini tasimaya devam eder.
- Validation ve promotion safety tarafina dokunulmadi.
...
## Son IntegrationRepository Risk Review

30 Nisan 2026 itibariyla `backend/nestjs/src/modules/integration/infrastructure/integration.repository.ts` icin backend repository risk incelemesi kayda alindi.
...
- Dosya 2322 fiziksel kaynak satiri ve source governance, raw staging write, KPI import store scope, import batch evidence, retry/action queue, external ID mapping audit ve import audit akisini birlikte tasiyor.
- Bu bir aktif bug degil; import/source/evidence persistence boundary icin bakim ve ileride performans riski.
- Import/source/evidence tarafi 32 e2e testle uc dosyada guard altinda: `import-batch.e2e-spec.ts`, `import-batch-evidence.e2e-spec.ts`, `integration-sources.e2e-spec.ts`.
- Raw table query scale icin en buyuk risk gercek veri hacmi gelince ortaya cikabilir; simdi speculative index eklenmeyecek.
- Karar: simdi refactor yok. `IntegrationRepository` split'i ve raw import index review planli yatirim; yalnizca somut integration/source degisikligi veya gercek row-volume kaniti geldiginde ele alinacak.
...
## Son StoreOpsRepository Risk Review

30 Nisan 2026 itibariyla `backend/nestjs/src/modules/store-ops/infrastructure/store-ops.repository.ts` icin backend repository risk incelemesi kayda alindi.
...
- Dosya 2008 fiziksel kaynak satiri ve seller-code request, offboarding request, store scope listing, target personnel rows, headcount gap ve legacy checklist instance write akisini birlikte tasiyor.
- Bu bir aktif bug degil; personel/workforce lifecycle persistence boundary icin bakim ve ileride performans riski.
- Seller-code flow 7 e2e, offboarding flow 5 e2e, checklist-flow 5 e2e ve auth scope/action-scope testleriyle guard altinda.
- En yuksek blast radius seller-code approval ve offboarding approval akislari; employee, assignment, request, turnover ve audit tablolarini transaction icinde etkiliyor.
- Audit actor bilgisi workforce request eventlerinde `metadata_json.actorUserId` icinde tutuluyor, checklist legacy eventlerinde `actor_user_id` kolonuna yaziliyor; future global audit feed icin planli iyilestirme adayi.
- Region queue indexleri icin simdi speculative index eklenmeyecek; gercek pilot hacmi veya slow query evidence beklenir.
- Karar: simdi refactor yok. `WorkforceRequestRepository` split'i planli yatirim; yalniz concrete workforce/personnel degisikligi geldiginde ele alinacak.

Siradaki mantikli adim: Dis staging/source/master-data kaniti yoksa `reporting.repository.ts` icin read-heavy boundary risk review yapmak; dis kanit gelirse onu onceliklendirmek.

## Son WorkforceRequestRepository Split V1 Design

30 Nisan 2026 itibariyla StoreOpsRepository risk review sonrasinda workforce request boundary split icin design spec yazildi.

Referans:

- `docs/superpowers/specs/2026-04-30-workforce-request-repository-split-v1-design.md`

Karar:

- Bu is artik panik refactor veya belirsiz planli yatirim degil; kontrollu mimari temizlik olarak ele alinacak.
- Yeni `WorkforceRequestRepository` yalniz seller-code ve offboarding request persistence sorumlulugunu alacak.
- `StoreOpsRepository` store scope listing, target personnel rows, headcount gap ve legacy checklist instance write sorumluluklarini tasimaya devam edecek.
- DB schema, migration, index, DTO/controller/auth, checklist, target distribution, org/store scope, headcount gap ve master-data davranisi degismeyecek.
- Audit actor kolon tutarsizligi, region queue indexleri, pagination ve global audit feed V1 disinda kalacak.

Uygulama oncesi kanit kapisi:

- `backend/nestjs`: workforce seller-code ve offboarding e2e testleri
- `backend/nestjs`: `store-ops.repository.spec.ts`
- `backend/nestjs`: build
- root: script guard, `check:release`, `git diff --check`

Siradaki mantikli adim: Bu design spec onaylanirsa `writing-plans` ile implementation plan yazmak; sonra sadece mekanik repository split uygulamak.
...
- `WorkforceService` seller-code/offboarding cagirilari icin yeni repository'yi kullanir.
- `StoreOpsRepository` store scope listing, target personnel rows, headcount gap ve legacy checklist instance write sorumluluklarini tasimaya devam eder.
...
- `backend/nestjs`: `npm.cmd run build` gecti.
- `backend/nestjs`: `npm.cmd test -- --runInBand test/integration/workforce-seller-code.e2e-spec.ts test/integration/workforce-offboarding.e2e-spec.ts` gecti, 12/12.
- `backend/nestjs`: `npm.cmd test -- --runInBand src/modules/store-ops/infrastructure/store-ops.repository.spec.ts` gecti, 2/2.
- root: `node --test scripts\*.test.mjs` gecti, 100/100.
- root: `git diff --check` gecti; yalniz Windows LF -> CRLF uyarilari var.
- root: `npm.cmd run check:release` gecti; backend 88 suite / 476 test, frontend Playwright 46/46, audit 0 vulnerability.

Siradaki mantikli adim: Dis staging/source/master-data kaniti yoksa `reporting.repository.ts` read-heavy boundary risk review yapmak; dis kanit gelirse onu onceliklendirmek.

## Son ReportingRepository Risk Review

30 Nisan 2026 itibariyla `backend/nestjs/src/modules/store-ops/infrastructure/reporting.repository.ts` icin read-heavy boundary risk incelemesi kayda alindi.
...
- Dosya 1897 fiziksel kaynak satiri ve 1737 non-empty satir tasiyor.
- Repository snapshot run lookup/list/count, workforce/KPI/checklist/turnover report read modelleri, store/personel live KPI performansi, Turkiye benchmark aggregations, store score breakdown inputlari, closed daily/monthly ranking, leaderboard ve auth identity -> employee lookup helperlarini birlikte tasiyor.
- Bu bir aktif bug degil; reporting/read-model boundary icin bakim ve ileride veri hacmi/performance riski.
- Reporting repository unit testleri empty-scope fail-closed davranisi, benchmark weighted-total formulleri, target reference joinleri ve store score breakdown inputlarini koruyor.
- Reporting/snapshot/auth integration testleri snapshot report read modellerini, closed daily/monthly ranking akisini, JWT/report viewer access'i ve region scope kisitlarini koruyor.
- En buyuk gelecek risk aylik closed ranking aggregate sorgulari, benchmark aggregations, large snapshot count queries ve real pilot volume ile ortaya cikabilecek index ihtiyaci.
- Karar: simdi refactor yok. `ClosedRankingReadRepository`, `PerformanceReadRepository` ve `SnapshotReportRepository` splitleri planli yatirim; yalniz concrete reporting/ranking degisikligi veya measured slow-query kaniti geldiginde ele alinacak.
- Auth identity lookup helperlari simdi tasinmayacak; auth/employee identity read-model boundary karari ayrica verilmeli.

Siradaki mantikli adim: Dis staging/source/master-data kaniti yoksa yeni production code acmadan `auth-admin.repository.ts` icin boundary risk review yapmak; dis kanit gelirse onu onceliklendirmek.

## Son AuthAdminRepository Risk Review + Pagination Count Fix

30 Nisan 2026 itibariyla `backend/nestjs/src/modules/auth/auth-admin.repository.ts` icin auth-admin boundary risk incelemesi kayda alindi ve incelemede bulunan kucuk pagination count kusuru kapatildi.
...
- Dosya 1769 fiziksel kaynak satiri ve 1647 non-empty satir tasiyor.
- Repository role lookup, user account create/list/deactivate/reactivate/audit, provider subject lookup, active employee access context, role assignment, action-store assignment, pilot user binding, role/permission catalog ve role permission grant/revoke sorumluluklarini birlikte tasiyor.
- Auth admin alani hassas oldugu icin genis refactor simdi yapilmadi.
- `AuthAdminController` default `SUPER_ADMIN` korumasinda; `POST /auth/pilot-user-bindings` ayrica `HR_ADMIN` icin acik.
- Runtime auth context `AuthAuthorizationRepository` uzerinden ayri ilerliyor; admin write repository request-time auth resolution'a karismiyor.
- `listUserAccounts` count sorgusunun `LIMIT/OFFSET` parametreleriyle kirlenmesi TDD ile yakalandi ve duzeltildi.
- `auth-user-accounts.e2e-spec.ts` artik 7 test tasiyor ve second-page count metadata davranisini koruyor.
- `ops.user_action_store_assignment` icin active unique index var; `ops.user_role_assignment` icin active role assignment uniqueness de artik DB seviyesinde nullable-scope-safe unique index ile korunuyor.
- Future scheduled/finite overlapping role assignment davranisi V1 disinda tutuldu; gercek workflow olursa ayri range/exclusion constraint plani gerekir.
- Auth-admin lookup limitleri pilot icin yeterli ama 900-1000 kullanici genislemesinde searchable lookup endpointlerine donusmeli.

Dogrulama:

- `backend/nestjs`: `npm.cmd test -- --runInBand test/integration/auth-user-accounts.e2e-spec.ts` gecti, 7/7.

Siradaki mantikli adim: Dis staging/source/master-data kaniti yoksa auth tarafinda genis split degil, once `role assignment active uniqueness` icin kucuk design plan cikarmak. Bu migration gerektirebilecegi icin once kural netlestirilecek, sonra uygulanacak.

## Son Auth Role Assignment Active Uniqueness V1 Design Plan

30 Nisan 2026 itibariyla role assignment aktif tekilligi icin implementation plani yazildi.
...
- `AuthAdminService` icindeki duplicate pre-check korunacak.
- DB seviyesi final guard olarak `ops.user_role_assignment` icin `uq_user_role_assignment_active_scope` partial unique functional index eklenecek.
- `company_id`, `region_id` ve `store_id` nullable oldugu icin normal unique index kullanilmayacak; `COALESCE(..., '00000000-0000-0000-0000-000000000000'::uuid)` ile normalize edilecek.
- Migration mevcut duplicate open-ended active role assignment satiri bulursa sessiz duzeltme yapmayacak; `RAISE EXCEPTION` ile duracak.
- V1 yalniz `end_at IS NULL` open-ended active role grants icin kilit koyacak.
- Future scheduled veya finite overlapping assignment davranisi bu slice disinda kalacak; gerekirse ayri range/exclusion constraint planiyla ele alinacak.

Dogrulama plani:

- Schema contract red/green.
- `auth-role-assignments.e2e-spec.ts`.
- Backend build.
- Root script guards.
- DB schema/migration degisecegi icin `npm.cmd run smoke:migration:fresh-db` veya written Conditional Go.
- Root `check:release`.

CODEX durust yorum:

- Bu is yeni feature degil, auth zemini guclendirme isi. Servis pre-check'i iyi ama coklu admin veya bulk provisioning oncesi DB kilidi olmadan tamamen rahat edemeyiz.
- Planin dogru siniri future scheduling'i bugun buyutmamak. Once bugunku open-ended role grant modelini saglama aliyoruz.

Siradaki mantikli adim: Bu plan onaylanirsa TDD ile schema contract + migration uygulamak ve fresh DB smoke dahil release kapilarini calistirmak.

## Son Auth Role Assignment Active Uniqueness V1 Implementation

30 Nisan 2026 itibariyla role assignment active uniqueness DB seviyesinde uygulandi.

Eklenenler:

- Yeni migration: `db/migrations/043_user_role_assignment_active_uniqueness.sql`.
- `ops.user_role_assignment` icin `uq_user_role_assignment_active_scope` partial unique functional index eklendi.
- Index `user_id`, `role_id`, `scope_type` ve nullable `company_id` / `region_id` / `store_id` alanlarini zero UUID sentinel ile normalize eder.
- Migration duplicate open-ended active role assignment bulursa sessiz veri duzeltmez; explicit `RAISE EXCEPTION` ile durur.
- Canonical `db/schema.sql` ayni index ile hizalandi.
- `AuthAdminService` duplicate pre-check'i ve mevcut API davranisi degismedi.
- V1 yalniz `end_at IS NULL` open-ended active role grants icin DB kilidi koyar; future scheduled veya finite overlapping assignment davranisi ayri gelecekteki slice olarak kalir.

Dogrulama:

- TDD RED: schema contract migration/schema yokken beklenen sekilde fail etti.
- `backend/nestjs`: `npm.cmd test -- --runInBand src/modules/auth/user-role-assignment-active-uniqueness-schema-contract.spec.ts` gecti, 3/3.
- `backend/nestjs`: `npm.cmd test -- --runInBand test/integration/auth-role-assignments.e2e-spec.ts` gecti, 9/9.
- `backend/nestjs`: `npm.cmd run build` gecti.
- root: `node --test scripts\*.test.mjs` gecti, 100/100.
- root: `npm.cmd run smoke:migration:fresh-db` gecti; disposable local DB'de 43/43 migration succeeded, failed=0, audit=3, ops=39, rpt=10, stg=12.
- root: `npm.cmd run check:release` gecti; backend 89 suite / 480 test, frontend Playwright 46/46, audit 0 vulnerability.

CODEX durust yorum:

- Bu borc kapandi. Artik servis pre-check'i ilk savunma, DB unique index ise concurrency ve bulk provisioning icin final kilit.
- En dogru sinir korundu: scheduling problemini bugun buyutmedik, mevcut pilot/genisleme modelindeki open-ended role grants'i saglama aldik.

Siradaki mantikli adim: Auth tarafinda yeni kod acmadan once dis staging/source/master-data kaniti varsa onu ele almak; yoksa broad rollout yaklasmadan searchable auth-admin lookup tasarimini ayri intake ile planlamak.

## Son Auth Admin Searchable Lookups V1 Design

30 Nisan 2026 itibariyla dis master-data/source/staging kaniti bulunmadigi icin yeni veri adapteri veya master-data isine girilmedi. Broad user rollout oncesi kalan auth-admin olcek riski icin searchable lookup tasarimi yazildi.
...
- Mevcut `GET /api/auth/lookups` bundled dropdown endpoint'i uyumluluk icin kalacak.
- V1 yalniz user ve store lookup aramasini hedefler.
- Role ve permission kataloglari kucuk kaldigi icin simdilik aynen kalacak.
- Onerilen endpointler:
  - `GET /api/auth/lookups/users/search?q=<text>&limit=20`
  - `GET /api/auth/lookups/stores/search?q=<text>&limit=20`
- User search `username`, `email`, `provider_subject` uzerinden active user arar.
- Store search `store_code`, `store_name`, `region_name` uzerinden active store arar.
- Query en az 2 karakter olmali; bos/çok kisa query ilk 20 kaydi fallback olarak dondurmeyecek.
- V1'de schema/index migration yok; 900-1000 kullanici hedefi icin capped admin search yeterli kabul edildi. Index/trigram ancak measured slow-query evidence ile ele alinacak.
- Auth-admin repository split, role assignment, action-store assignment, Keycloak/OIDC, runtime auth ve mobile auth/session davranisi bu tasarimin disinda kalacak.

CODEX durust yorum:

- Bu dogru sonraki auth-admin sertlestirme adayi ama implementation icin once spec review/onay mantikli. Buyuk refactor degil; dropdown limitinden kaynaklanacak operator problemini onleyen kucuk bir arama yuzeyi.

Siradaki mantikli adim: Bu spec onaylanirsa implementation plan yazmak; once backend searchable endpoints, sonra gerekirse minimal frontend wiring.

## Son Auth Admin Searchable Lookups V1 Implementation Plan

30 Nisan 2026 itibariyla searchable auth-admin lookup icin implementation plan yazildi.

Referans:

- Spec: `docs/superpowers/specs/2026-04-30-auth-admin-searchable-lookups-v1-design.md`
- Plan: `docs/superpowers/plans/2026-04-30-auth-admin-searchable-lookups-v1.md`

Plan siniri:

- Backend-first ilerlenir.
- Yeni DTO: `SearchAuthLookupQueryDto`.
- Yeni endpointler:
  - `GET /api/auth/lookups/users/search?q=<text>&limit=20`
  - `GET /api/auth/lookups/stores/search?q=<text>&limit=20`
- Mevcut `GET /api/auth/lookups` davranisi uyumlu kalir.
- User search active user icinde `username`, `email`, `provider_subject` arar.
- Store search active store icinde `store_code`, `store_name`, `region_name` arar.
- Query minimum 2 karakter ister; `limit > 50` validation error dondurur.
- Schema/index migration, frontend wiring, runtime auth, Keycloak, role assignment ve master-data davranisi degismeyecek.
...
Bu siradaki adim kayda baglandi: Project Health Snapshot 2026-05-01 eklendi.

## Son Project Health Snapshot 2026-05-01

1 Mayis 2026 itibariyla proje sagligi ile pilot onayi ayrildi.
...
- Proje kod karmasasi nedeniyle bloklu degil; eksik dis kanit nedeniyle pilot kapisi bloklu.
- Snapshot bir closed active debt item degil, kontrol notudur.

Kayitli son dogrulama:

- root script guard: 122/122
- backend: 89/89 suite ve 486/486 test
- frontend Playwright: 47/47
- audit: 0 vulnerabilities
- closed active debts: 89

Hala bekleyen dis kanitlar:
...
- pilot user/scope evidence
- realistic data volume for measured DB performance/index review

CODEX durust yorum:

- Proje dagilmis degil. Bence saglikli kisimlar artik yerinde: release gate, test genisligi, auth/scope kontrolu, import semantigi, master-data staging ve operator evidence zinciri var.
- Pilot icin risk, kodun dagilmis olmasi degil; gercek staging/data kaniti gelmeden pilot acmaya calismak.
...
## Son HR Axis Staging Deployment Evidence

2 Mayis 2026 itibariyla proje ilk gercek internet staging hattina tasindi.

Marka/domain:

- Uygulama adi: HR Axis.
- Domain Cloudflare uzerinden alindi: `hr-axis.com`.
- Cloudflare registrar privacy/WHOIS redaction varsayilan olarak aktif.
- Auto-renew acik.

Canli staging yuzeyleri:

- Frontend staging domain: `https://staging.hr-axis.com`.
- Store route: `https://staging.hr-axis.com/store`.
- Backend Render service: `https://hr-axis-api.onrender.com/api`.
- Backend custom domain hedefi: `https://api-staging.hr-axis.com/api`.

Mevcut aktif runtime karari:

- Frontend Vercel uzerinde `staging.hr-axis.com` domaini ile calisiyor.
- Vercel `VITE_API_BASE_URL` gecici olarak `https://hr-axis-api.onrender.com/api`.
- `api-staging.hr-axis.com` Render tarafinda verified/certificate issued oldu; public DNS `CNAME api-staging -> hr-axis-api.onrender.com` olarak dogru.
- Ancak kullanicinin tarayici/yerel DNS tarafinda `api-staging.hr-axis.com` henuz cozulmedigi icin frontend gecici olarak Render'in kendi domainine geri alindi.
- DNS tamamen oturunca `VITE_API_BASE_URL` tekrar `https://api-staging.hr-axis.com/api` yapilacak ve Vercel redeploy edilecek.

Backend/DB/auth durumu:

- Backend Render uzerinde NestJS web service olarak canli.
- DB Supabase Free Postgres.
- `db/seeds/001_reference_seed.sql` staging DB uzerinde tamamlandi.
- Backend health DB check `ok` verdi.
- Queue backend staging icin `in-memory`; Redis check bu nedenle skipped.
- Render `CORS_ALLOWED_ORIGINS` degeri yeni frontend domainini icerir:
  - `https://hr-axis-staging.vercel.app`
  - `https://staging.hr-axis.com`
- Clerk authentication aktif.
- Frontend bearer mode aktif.
- Clerk JWT template: `hr-axis-api`.
- Authorization uygulama DB'sinde kalmaya devam ediyor.

Alinan kanitlar:

- `https://staging.hr-axis.com/store` canli sayfa olarak acildi.
- Frontend bundle icinde gecici API base `https://hr-axis-api.onrender.com/api` olarak dogrulandi.
- Eski custom API base `api-staging.hr-axis.com` bundle icinde kalmadi.
- Render CORS preflight, `Origin: https://staging.hr-axis.com` icin `204 No Content` ve `access-control-allow-origin: https://staging.hr-axis.com` dondu.
- Backend health direct Render domaininde `{"status":"ok","service":"hr-axis-staging-api"}` dondu.
- Backend custom domain public DNS ve forced TLS testinde health `ok` dondu; sorun sadece yerel DNS/cache yayilimi.
- Clerk login daha once calisti.
- Clerk user DB authorization'a maplendi:
  - DB user id: `80000000-0000-0000-0000-000000000900`
  - Resolved roles: `SUPER_ADMIN, STORE_MANAGER`
  - Resolved scope: `1 company / 1 region / 1 store`
- `codex/fix-auth-seed-uuid` ve `codex/fix-reporting-seed-uuid` fixleri main'e merge edilip Render main redeploy sonrasi store auth/scope ve `/store/me` seed UUID kaynakli hatalari kapatti.

Staging smoke checklist:

- `https://staging.hr-axis.com/store` acilir.
- Clerk ile giris yapilir.
- Store shell'de resolved roles/scope beklenen degerleri gostermeli.
- Tekrar kontrol edilecek route'lar:
  - `/store`
  - `/store/me`
  - `/store/kpis`
  - `/store/approvals`
- `api-staging.hr-axis.com/api/health/live` kullanici tarayicisinda acilinca Vercel API base custom domain'e geri alinacak.

Guvenlik notu:

- Repo public oldugu fark edildi ve private'a alindi.
- Ilk manuel secret scan'de tracked gercek `.env` bulunmadi.
- `backend/nestjs/.env` local dosya olarak var ama tracked degil ve git history'de gorunmedi.
- Bulunan secret benzeri degerler docs/example/test placeholder olarak gorundu.
- Buna ragmen ileride GitHub secret scanning veya `gitleaks` gibi dedicated scanner kosulmasi mantikli.
- Her yeni provider secret'i yalnizca platform secret/env alanlarina girilmeli; repo, issue, chat veya screenshot icine yazilmamali.

Pilot kapisi etkisi:

- Bu adim staging auth/deploy kanitini ciddi sekilde ilerletti ama pilot `Go` degildir.
- Hala ayrica true store/personnel baseline kaniti, real KPI import smoke kaniti, pilot user/scope kaniti ve migration/release evidence tamamlanmali.

Siradaki mantikli adim:

- Kisa vadede staging smoke checklist tekrar kosulsun.
- `api-staging.hr-axis.com` kullanici tarayicisinda acilinca Vercel `VITE_API_BASE_URL` custom API domain'e geri alinsin.
- Sonra Pilot Readiness Gate V1 icin eldeki staging auth evidence kayda baglansin; true baseline ve KPI import kanitlari gelmeden pilot onayi verilmesin.

## Son Staging Smoke Recheck Evidence

2 Mayis 2026 15:10 +03 itibariyla staging public smoke tekrar kosuldu ve sanitized evidence olarak kayda baglandi.

Yeni kanit kaydi:

- `docs/evidence/pilot-readiness/2026-05-02-staging-smoke-recheck.md`

Kosulan kontroller:

- DNS:
  - `staging.hr-axis.com` Vercel DNS uzerinden cozundu.
  - `api-staging.hr-axis.com` artik bu workspace'ten `hr-axis-api.onrender.com` hedefine cozunuyor.
  - `hr-axis-api.onrender.com` Render origin uzerinden cozundu.
- Frontend HTTP:
  - `/`, `/store`, `/store/me`, `/store/kpis`, `/store/approvals` route'lari `200` HTML dondu.
- Backend health:
  - Render direct `https://hr-axis-api.onrender.com/api/health/live` `200 ok` dondu.
  - Render direct `https://hr-axis-api.onrender.com/api/health` `200 ok`, database `ok`, Redis `skipped`, queue backend `in-memory` dondu.
  - Custom API `https://api-staging.hr-axis.com/api/health/live` `200 ok` dondu.
  - Custom API `https://api-staging.hr-axis.com/api/health` `200 ok`, database `ok`, Redis `skipped`, queue backend `in-memory` dondu.
- Auth/CORS:
  - `/api/auth/session` bearer token olmadan iki API domaininde de `403` dondu.
  - `OPTIONS /api/auth/session`, `Origin: https://staging.hr-axis.com` icin iki API domaininde de `204` dondu.
  - `access-control-allow-origin` degeri `https://staging.hr-axis.com`.
- Browser route smoke:
  - Chromium ile `/`, `/store`, `/store/me`, `/store/kpis`, `/store/approvals` acildi.
  - Unauthenticated store route'lar `/auth/login` kapisina dustu.
  - Console error ve request failure gorulmedi.
  - Login ekrani Clerk provider flow'u configured olarak gosterdi.
- Frontend bundle:
  - Deployed bundle hala gecici `https://hr-axis-api.onrender.com/api` base'ini iceriyor.
  - Deployed bundle henuz `https://api-staging.hr-axis.com/api` base'ini icermiyor.
  - Clerk JWT template marker `hr-axis-api` bundle icinde mevcut.

Vercel custom API base redeploy durumu:

- Custom API domain bu workspace'ten artik health/CORS seviyesinde calisiyor.
- Lokal workspace'te `.vercel` link dizini, `vercel` CLI veya `VERCEL_*` token/env bulunmadi.
- Bu nedenle Vercel `VITE_API_BASE_URL` bu turda otomatik custom API domain'e alinip redeploy edilemedi.

Pilot kapisi etkisi:

- Bu public staging smoke iyi bir ara kanittir ama pilot `Go` degildir.
- Authenticated Clerk login, real bearer ile `/api/auth/session`, store shell resolved role/scope, assigned-store positive action ve unassigned-store negative scope smoke bu turda kosulmadi.
- True baseline master data, real KPI import smoke, pilot user/scope ve release/migration evidence hala zorunlu.

CODEX durust yorum:

- Staging hattinin dis kabugu artik saglikli gorunuyor: custom API domain de calisiyor, backend DB health ok, CORS dogru, protected route'lar login kapisina temiz dusuyor.
- Asil pilot riski artik DNS degil; otomasyonla kanitlanmamis authenticated Clerk session ve gercek data/scope kaniti.
- Vercel API base custom domain'e gecmeye teknik olarak hazir gorunuyor, ama bu workspace'te Vercel deploy yetkisi olmadigi icin aksiyon dashboard/CLI yetkisi olan tarafta tamamlanmali.

## Son Clerk Token Refresh Shell Stability Fix

2 Mayis 2026 itibariyla staging frontend'de kullanici sayfada beklerken tam 45 saniyede bir gorunen otomatik yenilenme hissinin root cause'u bulundu ve fixlendi.

Root cause:

- `ClerkSessionBridge` her 45 saniyede Clerk token sync calistiriyor.
- Token string'i yenilendiginde `startBearerSession(token)` React session state'ini guncelliyordu.
- `App.tsx` shell session query key'i raw `session.bearerToken` degerini iceriyordu.
- Bu nedenle her Clerk token refresh'i `GET /auth/session` shell verification query'sini yeni query gibi tetikliyor, store shell'i tekrar verifying/yol secimi moduna sokuyordu.

Fix:

- `admin-web/src/App.tsx` icinde shell session query key'i raw bearer token yerine yalnizca `token-present` / `token-missing` readiness bilgisini kullanacak sekilde degistirildi.
- Token refresh hala sessionStorage'a en guncel token'i yazar; API request header'lari zaten request aninda sessionStorage'dan okundugu icin authorization tazeligini kaybetmez.
- 45 saniyelik Clerk token refresh artik shell query key'i degistirmez; sayfa/store shell gereksiz verification state'e dusmez.

Regression guard:

- Yeni root script testi: `scripts/frontend-auth-refresh-contract.test.mjs`.
- Guard, shell session query key icinde raw `session.bearerToken` tekrar kullanilirsa fail eder.

Dogrulama:

- TDD RED: `node --test scripts\frontend-auth-refresh-contract.test.mjs` once raw bearer token query key'de oldugu icin fail etti.
- GREEN: targeted guard gecti.
- Frontend build: `admin-web` `npm.cmd run build` gecti.
...
- Kullanici 3 Mayis 2026'da dosya donemini `monthly` Mart 2026 olarak teyit etti: `periodMonth=2026-03`, `periodStart=2026-03-01`, `periodEnd=2026-03-31`.
...
- Backend `RoleGuard` `SUPER_ADMIN` icin global bypass verdigi icin `INTEGRATION_ADMIN` role satiri tek basina upload blocker degildir.

Pilot kapisi etkisi:

- Real KPI file preflight kapisi ilerledi; dosyalar KPI snapshot olarak uygun gorunuyor.
- Official KPI import/materialization hala `No-Go`; staging import scope, authenticated upload, batch id/source batch id, unmapped counts ve backend reconciliation evidence tamamlanmali.

## Son Power BI Export Upload Unblock

3 Mayis 2026 itibariyla `/admin/integrations` uzerinde `Power BI export yukle` butonunun "tikliyorum ama hareket yok" hissi icin root cause ve urun fix'i eklendi.

Yeni kanit kaydi:

- `docs/evidence/pilot-readiness/2026-05-03-power-bi-upload-unblock.md`

Root cause:

- Upload butonu missing prerequisite durumunda disabled kaliyordu.
- UI buton yaninda disabled nedenini gostermedigi icin operator bunu no-op gibi goruyordu.
- Reference seed'de `power_bi` / `kpi` source yoktu; sadece `HRIS` ve `POS` source'lari vardi.
- Backend role mismatch root cause degil: `RoleGuard` `SUPER_ADMIN` icin global bypass veriyor.

Fix:

- Yeni migration: `db/migrations/045_power_bi_kpi_source_seed.sql`.
- Seed'e default active `power-bi-kpi` source eklendi:
  - `source_system=power_bi`
  - `entity_type=kpi`
  - `state_model=closed_period`
- `/admin/integrations` upload alanina readiness mesajlari eklendi:
  - aktif Power BI KPI source yok
  - personel veya magaza Excel dosyasi secilmedi
  - source secimi yok
  - donem bilgisi gecersiz
- Regression guard eklendi:
  - `scripts/default-power-bi-source-contract.test.mjs`
  - `admin-web/e2e/integration-surfaces.spec.ts` disabled upload explanation testi

Dogrulama:

- TDD RED: default Power BI source kontrati seed/migration yokken fail etti.
- TDD RED: Power BI upload disabled explanation Playwright testi UI mesaji yokken fail etti.
- GREEN: default Power BI source kontrati gecti.
- GREEN: `admin-web` build gecti.
- GREEN: targeted Playwright upload explanation testi gecti.
- Wider: root script tests `124/124` gecti.
- Wider: integrations Playwright surface `5/5` gecti.
- `git diff --check` gecti; yalniz line-ending warning'leri var.
- `npm.cmd run smoke:migration:fresh-db` kosulamadi; Docker Desktop daemon kapali oldugu icin Postgres image alinamadi.

Sinir:

- Staging DB migration bu turda uygulanmadi.
- Staging frontend/backend deploy bu kayitla kanitlanmadi.
- Official authenticated Power BI upload smoke hala kosulmadi.
...
## Son Staging Token Scope Smoke Harness

3 Mayis 2026 itibariyla Clerk token tabanli live API session/action-scope smoke icin harness eklendi; live token olmadigi icin smoke henuz tamamlanmadi.
...
- Yeni script: `admin-web/scripts/auth-token-scope-smoke.mjs`
- Yeni test: `admin-web/scripts/auth-token-scope-smoke.test.mjs`
- Yeni npm komutu: `npm.cmd --prefix admin-web run smoke:auth:staging:token-scope`

Script davranisi:

- `AUTH_SMOKE_BEARER_TOKEN` local env uzerinden alinir ve stdout/stderr'a basilmaz.
- `GET /auth/session` ile sanitized session/role/scope kaniti alir.
- `GET /target-distributions/store-personnel?storeId=<assignedStoreId>` ile read-only assigned action-scope positive smoke kosar.
- Ayni endpointi unassigned store id ile cagirip `403` bekler.
- Write/mutation yapmaz; target request olusturmaz.
...
- Public health: `https://api-staging.hr-axis.com/api/health` `200 ok`, database `ok`.
- Unauthenticated `/api/auth/session`: `403`, beklenen protected boundary.
- `/api/auth/bootstrap`: `authMode=jwt`, OIDC provider bootstrap `configured=false`; bu nedenle eski `auth-live-smoke.mjs` OIDC browser flow'u Clerk modal staging icin dogru yol degil.
- Token-scope script staging'de token yokken `AUTH_SMOKE_BEARER_TOKEN is required` ile fail-fast oldu.
- `npm.cmd --prefix admin-web run test:scripts` gecti: `10/10`.

Bloker:

- Shell environment'ta `AUTH_SMOKE_BEARER_TOKEN` yok.
- Reusable staging login automation credential yok.
- Codex in-app browser `about:blank` tab acsa da staging URL'e giderken `failed to start codex app-server: Sistem belirtilen yolu bulamiyor. (os error 3)` ile calismadi.
- Bu yuzden live authenticated `/api/auth/session` payload ve assigned/unassigned action-scope API evidence hala acik.

Siradaki mantikli adim:

- Kullanici staging'de authenticated browser session acikken tokeni chat/docs'a yazmadan yalniz local env'e koyar.
- Sonra `npm.cmd --prefix admin-web run smoke:auth:staging:token-scope` kosulur ve sanitized JSON output evidence olarak kayda baglanir.
