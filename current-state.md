...
Now:
- The owner-approved photo-evidence train is active through merged PR #1025 (`c1b89c1a2f2cbf2ee87c161c33df6b5d929efe00`). PR #1025 delivered VM reference management and the product owner reported representative physical iOS Safari and Android Chrome interactions passed on 28 July 2026. PR-7 retention operations are the active repository slice: digest-bound dry-run manifests, exact execution, lifecycle reconciliation, usage thresholds, derived-artifact policy pinning, and a pre-use rollback are being added while scheduled cleanup remains disabled. This implementation does not authorize a purge or activate staging deletion. All VM grants and runtime flags remain disabled by default. Exactly one byte-identical fixture remains admitted and rechecked by SHA-256; this is `fixture_identity_only`, not malware scanning. No Render ClamAV service is authorized, camera and arbitrary gallery media remain disabled, and real photographs remain No-Go until a separately approved and proven malware-scanner adapter exists. The R2 posture remains two distinct private Standard EU buckets, separate bucket-scoped credentials, verified recovery before `ready`, 8 GiB / 750k Class A / 7.5m Class B ceilings, 30-day `locked/` safety window, RPO 0 and manual RTO <=24h. No paid activation or broad-production authority exists. See `docs/plans/checklist-photo-media-storage-provider-decision-v1.md`, `docs/plans/vm-reference-management-pr6-execution-spec-v1.md`, and `docs/plans/photo-media-retention-operations-pr7-execution-spec-v1.md`.
...
Next:
1. Finish PR-7 repository verification and merge only with cleanup disabled, clean mergeability, and all required checks green. Do not execute a purge.
2. After PR-7, stop at the PR-8 provider/data gate unless approved candidate sandboxes, privacy/data terms, and a frozen double-reviewed labelled benchmark dataset exist.
...
4. Keep ORG/ASSIGN correction, DG1-C contraction, real photographs, AI provider activation, paid services, and broad production behind their recorded gates.
Park:
