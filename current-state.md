...
Next:
1. Merge the docs-only PR-8A Qwen decision alignment, then stop before PR-8B until the exact Alibaba workspace region/deployment scope and endpoint, API-key custody, DPA/privacy/no-training/retention/deletion/subprocessor evidence, incident owner, cost ceiling, frozen digest-bound double-reviewed dataset and holdout, and owner-approved acceptance thresholds exist.
