...
Next:
1. Start logical PR-8B from fresh `main` under the lean store-photo pilot decision. Implement the provider-neutral port, exact Qwen adapter, strict schema and failure contracts, fail-closed cost/timeout controls, and operator-run 20-pair synthetic smoke without product writes or UI. Provider invocation still requires the server-side secret and exact-model/budget assertions; raw images and secrets never enter Git.
