...
Now:
- The owner-approved photo-evidence train is active. PR #1020 locked the product/provider gates, PR #1021 merged the provider-neutral schema foundation, PR #1022 merged disabled-by-default synthetic private-media storage, PR #1023 merged checklist item evidence, and PR #1024 merged Store Action photo review V2 at `d3bd8a81f9f40c6e3559266df49968b5de06d152`. PR-6 VM reference management is locally verified on `codex/vm-reference-management-v1`: immutable reference versions and assignment snapshots, exact company/store/template-item authorization, independent window/scope/emergency capabilities, payload-bound idempotency, deadline settlement, retention-aware retirement, typed OpenAPI, and the Store Manager campaign surface are implemented. The product owner reported representative physical iOS Safari and Android Chrome interactions passed on 28 July 2026. All VM grants and runtime flags remain disabled by default; this is repository merge evidence only and does not authorize staging mutation or activation. Exactly one byte-identical fixture remains admitted and rechecked by SHA-256; this is `fixture_identity_only`, not malware scanning. No Render ClamAV service is authorized, camera and arbitrary gallery media remain disabled, and real photographs remain No-Go until a separately approved and proven malware-scanner adapter exists. The R2 posture remains two distinct private Standard EU buckets, separate bucket-scoped credentials, verified recovery before `ready`, 8 GiB / 750k Class A / 7.5m Class B ceilings, 30-day `locked/` safety window, RPO 0 and manual RTO <=24h. No paid activation or broad-production authority exists. See `docs/plans/checklist-photo-media-storage-provider-decision-v1.md`, `docs/plans/store-action-photo-review-pr5-execution-spec-v1.md`, and `docs/plans/vm-reference-management-pr6-execution-spec-v1.md`.
