...
Now:
- The owner-approved photo-evidence train is complete through logical PR-8A, squash-merged as PR #1028 (`ce1df5c67fd61fc5b9692ad3b54d6dc2c02cb801`). PR #1025 delivered VM reference management and the product owner reported representative physical iOS Safari and Android Chrome interactions passed on 28 July 2026. PR #1026 delivered digest-bound retention previews, exact governed execution, lifecycle reconciliation, usage thresholds, derived-artifact policy pinning, typed failures, and pre-use rollback; final R5 review, required checks, Post-Merge Verification, and Release Rehearsal passed. PR #1027 closed the PR-7 handoff, and PR #1028 locked the Qwen decision and PR-8B external gates without runtime changes. Scheduled cleanup remains disabled and no purge or staging deletion was executed. All VM grants and runtime flags remain disabled by default. Exactly one byte-identical fixture remains admitted and rechecked by SHA-256; this is `fixture_identity_only`, not malware scanning. No Render ClamAV service is authorized, camera and arbitrary gallery media remain disabled, and real photographs remain No-Go until a separately approved and proven malware-scanner adapter exists. The R2 posture remains two distinct private Standard EU buckets, separate bucket-scoped credentials, verified recovery before `ready`, 8 GiB / 750k Class A / 7.5m Class B ceilings, 30-day `locked/` safety window, RPO 0 and manual RTO <=24h. No paid activation or broad-production authority exists. On 28 July 2026 the owner selected Alibaba Model Studio Qwen `qwen3.7-plus-2026-05-26` as the single visual-comparison model, non-thinking with strict structured output and no tools or second model; the responsible Region Manager retains final human review. This closes candidate choice only. Provider access, real-photo transfer, paid activation, staging execution, and product integration remain gated. See `docs/plans/checklist-photo-media-storage-provider-decision-v1.md`, `docs/plans/vm-reference-management-pr6-execution-spec-v1.md`, `docs/plans/photo-media-retention-operations-pr7-execution-spec-v1.md`, and `docs/plans/qwen-visual-comparison-pr8-execution-spec-v1.md`.
...
Next:
1. Treat logical PR-8B as `blocked_external` after merged PR-8A (#1028). Do not implement or invoke the adapter until the exact Alibaba workspace region/deployment scope and dedicated endpoint, pinned-snapshot availability, API-key custody, accepted DPA/privacy/no-training/retention/deletion/subprocessor evidence, incident owner, request/monthly/benchmark cost ceilings, frozen digest-bound double-reviewed dataset and holdout, and owner-approved quality/latency exit thresholds all exist.
...
4. Keep ORG/ASSIGN correction, DG1-C contraction, real photographs, Qwen runtime activation, paid services, and broad production behind their recorded gates.
Park:
