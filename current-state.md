...
- The first post-#955 attempt stopped before receipt because CA path was passed instead of PEM; PR #956's attempt stopped before connection on Windows `EINVAL`. Both SHA markers remain consumed. PR #957 merged the reviewed Windows wrapper, and its exact SHA produced one valid verify-full, repeatable-read/read-only V2 receipt: `TARGET-02=0`, `ORG-02=3`, `ASSIGN-01=4`, and `ORG-04=7142`. The large ORG-04 total is failed authority resolution, not permission to infer managers or bulk-edit period rows.
...
Next:
1. Merge the V2 receipt and adjacent evidence record without changing its already-executed query, runner, launcher, or tests. Preserve all three one-shot markers.
2. Build the minimum versioned read-only classifier needed to bind ORG-04 effective-manager authority, ORG-02 lifecycle authority, and ASSIGN-01 rotation winners; aggregate counts never select a correction row.
3. Preserve `TARGET-02` and carry its V2 zero into REM-7. Missing/ambiguous manager or rotation authority stays blocked; do not open correction packages until exact row evidence exists.
4. Keep DB-CONSTRAINTS blocked until terminal family classification, applicable manifest-bound corrections, reconciliation, and lock/restore/rollback gates close. Standing authorization removes repeated prompts but does not satisfy missing evidence.
