...
Now:
- ONP-5 foundation PR #1061 merged at `bdcee07df89346eef61ac05efa7d9cd76b668a50`; the final offline-package/rehearsal PR is active on `codex/onprem-offline-package-v2` as a repository/synthetic release candidate, not a production approval. The candidate includes an independently pinned self-contained bootstrap verifier, exact seven-image source-free bundle closure, the signed opt-in photo-proof and no-host-port proof Compose overlays, root-sealed operator inputs and backup outputs with canonical private ancestor checks, explicit install/migrate/activate/smoke/recovery phases, protected five-persona plus synthetic photo-admin HTTP proofs, a canonical root-private pre-backup photo recovery handle whose filesystem identity and hash are revalidated through restore/upgrade/rollback and whose exact asset hash/bytes are recovered, current-bundle-authoritative historical rollback compatibility, bounded streaming backup sealing/inventory verification, post-backup exact identity/health recovery with signed-backup retention on resume failure, and packet-counter-bound Docker/host IPv4/IPv6 egress denial. The workflow's cloned release IDs prove only `same-build-lifecycle-mechanics`; they do not prove a real previous/next version transition or incompatible-migration recovery. Fresh final focused evidence is a read-only local `node:24-trixie-slim` `--network none` Linux run `110 passed / 1 iptables-translate-unavailable skip / 0 failed`, Windows bootstrap/photo/target/workflow contracts `48 passed / 1 platform skip / 0 failed`, Windows rollback-authority/operator focused contracts `2 passed / 1 Linux-only skip / 0 failed`, and the existing on-prem suite `291 passed / 1 platform skip / 0 failed`; the aggregate Windows offline suite exceeded its eight-minute local harness limit without an assertion result, so it is not claimed. The real hosted workflow remains the iptables parser/runtime gate. The hosted clean-Linux offline workflow has not been run, independently produced transition bundles, owner trust distribution, and IT backup/host/RPO/RTO gates remain external; company-server activation, real data/photos, live provider access, and production continuity remain No-Go.
