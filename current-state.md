...
Next:
1. Merge the exact-SHA REM-8 evidence-only PR after its receipt validators, evidence-scope guard, and canonical docs checks pass.
2. Open only TARGET-owned DB-C5 enforcement from that merge; reuse the exact candidate digests and omit REM-8C.
3. Keep DB-C5 staging apply reversible and separately evidenced; no production operation enters this train.
4. Keep ORG-02, ORG-04, and ASSIGN-01 correction/constraint work blocked until exact historical authority evidence exists; never infer winners from current timestamps.
