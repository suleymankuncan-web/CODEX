...
Now:
- ONP-5 foundation PR #1061 merged at `bdcee07df89346eef61ac05efa7d9cd76b668a50`; final offline-package/rehearsal closure is active locally on `codex/onprem-offline-preflight-closure-v1` from `527844ce091dfbd56d3f82c81301a99dd0425b81`, with no PR/merge claim yet. The candidate includes an independently pinned self-contained bootstrap verifier, exact seven-image source-free bundle and raw vendor-evidence closure, signed opt-in photo-proof and no-host-port proof Compose overlays, root-sealed operator inputs and backup outputs with canonical private ancestor checks, explicit install/migrate/activate/smoke/recovery phases, protected five-persona plus synthetic photo-admin HTTP proofs, a canonical root-private pre-backup photo recovery handle whose filesystem identity and hash are revalidated through restore/upgrade/rollback and whose exact asset hash/bytes are recovered, current-bundle-authoritative historical rollback compatibility, bounded streaming backup sealing/inventory verification, post-backup exact identity/health recovery with signed-backup retention on resume failure, and packet-counter-bound Docker/host IPv4/IPv6 egress denial. Customer delivery is one mode-preserving, signed `current.tar`; the real synthetic proof archive measured `1,021,460,480` bytes (about 974 MiB). `next-transition.tar` and `previous-transition.tar` measured `30,720` bytes each and exist only to prove same-build CI lifecycle mechanics; they are not customer updates or real rollback releases. Delivery may use removable media or an approved VPN/SSH/SFTP channel, but verification and every Docker operation execute locally on the protected host through an independently hashed Node runtime; the Docker daemon is not exposed over an unauthenticated remote socket. A real rollback retains an independently built and signed previous release, and this same-build proof does not establish a real version transition or incompatible-migration recovery. Fresh real-artifact evidence includes Windows staging against the 2.06 GB proof root, semantic attribution checks against all four real vendor SBOM/Trivy/license sets, Linux network-none producer plus self-contained bundle verification, and executable traversal/fingerprint/poisoned-system-Node negatives. The settled diff received a final R5 GO/CLEAN with no P1/P2. Exact-diff canonical release, PR/hosted checks, and merge identity must still be verified live before merge; independently produced transition bundles, owner trust distribution, and IT backup/host/RPO/RTO gates remain open. Company-server activation, real data/photos, live provider access, and production continuity remain No-Go.
