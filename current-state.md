...
- REM-2C is the versioned repository-only row-authority classifier: ORG-04 separates hierarchy/role/effective-date/portfolio/duplicate/multi-manager roots and deduplicates store-date authority units; ORG-02 and ASSIGN-01 fail closed because no approved rotation/lifecycle source contract exists. Disposable proof covers all ten reasons, `13 check_hits`, `12 authority_units`, and rollback with no staging access.
...
Next:
1. Merge REM-2C through normal checks, then create a fresh evidence-only branch from its exact merge SHA; implementation and staging receipt remain separate PRs.
2. Execute the merged classifier once with the existing target/project/CA/window/one-shot guards. Do not change query/runner/launcher on the evidence branch.
3. Use only that sanitized receipt to choose ORG-04 root packages. ORG-02/ASSIGN remain blocked when rotation authority is absent; aggregate/current-row timestamps never select winners.
4. Preserve `TARGET-02` for REM-7 and keep DB-CONSTRAINTS blocked until classification, applicable corrections, reconciliation, and lock/restore/rollback gates close.
