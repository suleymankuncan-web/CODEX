...
- Owner-authorized private-server company mode is implemented and locally verified (7 September 2026): explicit strict-local opt-in, company resource labels, synthetic initializer rejection, admin deep links and same-origin cookie recovery. Canonical release and isolated login/reload proofs passed; deployment data, credentials and cutover/restore receipts stay private. See `docs/contracts/onprem-company-data-mode-v1.md`. The next owner-authorized UI slice adds daily store-detail KPI selection with matching personnel reads; automatic source scheduling and personnel correction approval are separate work; correction requests now have a scoped submit/review implementation under verification, with no manager accounts provisioned.
