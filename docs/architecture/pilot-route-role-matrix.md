...
| `/store/workforce` | store | secondary | `STORE_MANAGER` with assigned action store, `REGION_MANAGER` with read store or read region scope, `REPORT_VIEWER` with non-empty company read scope | direct navigation only | must return to same route after auth verification | read-only company portfolio for Report Viewer; request/forms and action-assisted lookups remain unavailable | yes |
