...
| `/store` | store | core | authenticated store shell session | first landing family for store and region roles; current resolved landing is `/store/home` | must return to same route after auth verification | current store shell overview | yes |
