...
Production Readiness Decision Packet V1 is counted as paid because PR #240 merged `docs/evidence/readiness/2026-05-18-production-readiness-decision.md`, consolidates the readiness roadmap through PR #239 into one Go / Conditional Go / No-Go decision, links merged evidence, keeps broad production at No-Go, and names the external evidence required before controlled pilot expansion or broad rollout.
...
Staging Auth Session Edge Evidence Guard V1 is counted as paid because logout and expired-token behavior are now locked to the existing staging auth smoke and evidence guard path. It documents that `smoke:auth:staging`, `smoke:auth:staging:action`, and `guard:auth:evidence` must prove provider logout, token storage cleanup, no expired-token authorization header, no refresh-token use, and sanitized evidence before pilot expansion or broad rollout. Reference: `docs/plans/staging-auth-session-edge-evidence-guard-v1.md`.
...
AuthAdminRepository boundary split is a planned investment, not active hidden debt today. The current pilot path is guarded, active role assignment uniqueness is protected at the database level, and searchable user/store lookup endpoints now avoid dropdown-only limits before broader rollout. The admin UI wiring for role/action-store assignment is also implemented, so the remaining repository split should happen only when a concrete auth-admin change touches a clean family of operations. References: `docs/plans/auth-admin-repository-risk-review-2026-04-30.md`, `docs/superpowers/plans/2026-04-30-auth-role-assignment-active-uniqueness-v1.md`, `docs/superpowers/plans/2026-04-30-auth-admin-searchable-lookups-v1.md`.
