...
- Locked DG1-DG2 execution contract:
...
3. Preserve the persona sessions, Moi scope pass, and real company-scoped
   Report Viewer binding. The next safe B1 bundle is live period totals/export
   readback; account recovery and fresh live cross-person denial remain separate
   gates. Preserve the 5 July mutations; do not rerun only for documentation.
...
2. Dedicated integration-admin persona proof is no longer required for the
...
- Staging public health/deployed readiness are reachable. Redis/BullMQ staging
  wiring is now proven, and one Render Notifications delivery path is now
  proven through Slack.
...
- `GET /api/integrations/import-batches` previously returned HTTP `500` in
  ...
  overview endpoints.
- Supabase staging application-schema logical restore is now proven; Supabase
  managed backup/PITR/RPO/RTO policy, Redis/BullMQ broad-production posture,
  and production alert policy remain open until real inputs are supplied.
  Dedicated
  `INTEGRATION_ADMIN` proof is not required for the current pilot; future
  `HR_ADMIN` import/upload delegation would be a separate scoped auth change.
- Controlled Pilot Round 2 technical stabilization is recorded in
  ...
  local `check:pilot-stabilization` passed, staging deployed readiness was
  public-health green with auth/session skipped because no bearer token was
  provided, public backend load was healthy, and alert routing still lacks
  external provider delivery proof.
- `PILOT-005` store personnel approvals UX cleanup is recorded in
  `docs/evidence/pilot-readiness/2026-05-22-pilot-005-store-approvals-personnel-ux.md`:
  `STORE_PERSONNEL` no longer opens `/store/approvals` by direct route or via
  leftover personnel-facing links; manager/review approvals behavior stays
  unchanged.
- Production evidence closure preflight was rerun from the joint plan context:
  deployed readiness passed `13/14` with auth skipped because no bearer token
  was provided, `/api/health` still reported `queueBackend=in-memory`,
  Redis `skipped`, and alert routing passed `4/5` with provider delivery still
  `not-configured`.
- Redis/BullMQ staging proof is now recorded in
  `docs/evidence/readiness/2026-05-22-redis-bullmq-staging-proof.md`: after the
  operator connected Render Key Value to the staging backend, `/api/health`
  returned `queueBackend=bullmq`, queue `status=durable`, and Redis
  `status=ok`; broad production remains No-Go until a production-grade Redis
  tier/profile decision, alert provider delivery, and Supabase restore proof
  are also closed.
- Alert provider delivery proof is now recorded in
  `docs/evidence/readiness/2026-05-22-alert-provider-delivery-proof.md`: Render
  Notifications delivered a staging backend deploy notification to Slack after
  `/api/health` was set as the health check path, and `smoke:alert-routing`
  passed 5/5 with provider metadata plus deployed backend health. Email
  delivery was not observed and is not counted as proven. Broad production
  remains No-Go until Supabase restore, production Redis/profile posture, and a
  production alert policy/proof are also closed.
- Email delivery was not observed in the alert-provider proof; Slack is the
  proven external delivery path for this staging pass.
- Supabase staging logical restore proof is now recorded in
  ...
  PostgreSQL 17 target with `vault` excluded as a Supabase-managed extension;
  source and restore counts matched for app schemas, migration rows, and
  `ops.store`. Broad production remains No-Go until production Redis/profile,
  production alert policy/proof, and Supabase managed backup/PITR/RPO/RTO
  acceptance are also closed.
- Readiness profile reset evidence is now recorded in
  `docs/evidence/readiness/2026-05-22-readiness-profile-reset-after-broad-smoke.md`:
  the temporary `READINESS_PROFILE=broad-production` smoke proved the Redis /
  BullMQ technical path on staging but correctly surfaced observability
  `degraded` without a real `ERROR_TRACKING_DSN`. The backend was reset to
  `READINESS_PROFILE=controlled-pilot` with Redis/BullMQ still enabled; health,
  deployed readiness, public backend load, and alert routing are green again.
  Broad production remains parked until the owner chooses real app-level error
  tracking or explicitly accepts platform alerting, and until production-grade
  Redis plus managed backup/PITR/RPO/RTO policy are accepted.
