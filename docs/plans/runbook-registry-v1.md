...
| Execute security launch blocker PR train | `docs/plans/security-launch-blocker-pr-train-v1.md` | Owner decision to remove MVP/launch security blockers, clean branch, and later real provider session for evidence | Env contract, backend cookie session, frontend token-storage removal, CSRF guard, sanitized evidence, and closeout | Viewing/copying/pasting/logging/recording real raw secrets/tokens/cookies, provider config change, DB migration, role/scope semantic change, unapproved staging mutation, or mixed-domain PR. |
