...
| Execute pilot feedback PR train | `docs/plans/controlled-pilot-feedback-loop-pr-train-v1.md` | Feedback record, severity, route/persona, and PR authority | Intake PR, targeted blocker PR, optional same-surface P2 batch, or closeout decision | Secret handling, provider config, auth/DB/API behavior change, unapproved staging mutation, or mixed-domain PR. |
| Prepare pilot preflight | `docs/plans/controlled-pilot-preflight-checklist-v1.md` | Target persona, session scope, latest deploy state, and data/import context | Ready, Conditional Go, No-Go, or Parked decision before the pilot session starts | Nebim/source integration work is requested, target scope is unclear, or target route/auth/data evidence is unsafe. |
| Check launch browser-session evidence | `docs/plans/security-launch-blocker-pr-train-v1.md`, `docs/evidence/readiness/2026-06-12-security-launch-blocker-pr-train-closeout.md`, `docs/evidence/readiness/2026-06-12-browser-session-staging-evidence.md` | Approved real staging provider session or controlled browser session, expected role/landing, and sanitized smoke authority; rerun with `npm.cmd --prefix admin-web run smoke:auth:staging:cookie-session` | Sanitized cookie-session evidence or explicit blocked/historical note | Viewing/copying/pasting/logging/recording real raw secrets/tokens/cookies, DB migration, role/scope semantic change, unapproved staging mutation, fake proof, or mixed-domain PR. |
...
| Run pilot daily ops before external data | `docs/runbooks/pilot-daily-ops-runbook-v1.md` | Staging URLs, expected deploys, pilot personas, latest source batch state, and feedback owner | Daily start/end state, persona smoke status, import/data-quality evidence, and stop/rollback decision | Auth/scope is unclear, accepted/quarantine counts are not explainable, report exports are invalid, or provider/schema/API/formula work would be required. |
| Prepare or clean pilot access | `docs/plans/pilot-access-matrix-v1.md` | Target account label, intended persona, role/scope change reason, and approved secret-channel credential source | Sanitized account-class decision, protected-baseline check, active role/action-store counts, and persona smoke result | Cleanup would leave an active Clerk-linked account with zero active roles, require raw secrets in evidence, or mutate protected real account access without explicit owner approval. |
...
| Enable, verify, or roll back DG-4 Sentry delivery | `docs/runbooks/dg4-sentry-app-error-delivery-v1.md` | Sentry project, Render API/worker env boundary, Cloudflare frontend build env, and owner email destination | Sanitized API/worker/frontend event receipt, redaction result, release/environment labels, and rollback state | DSN appears in source/evidence, smoke is run outside controlled staging, or delivery is assumed without a receipt. |
...
| Triage Redis/BullMQ controlled-pilot queue symptoms | `docs/plans/redis-bullmq-controlled-pilot-runbook-v1.md` | Pending import/snapshot/job symptom plus public staging health | Pilot continue/pause decision and next operator action | Provider tier/settings, queue behavior, worker boot, data repair, or queue drain would be required. |
| Apply controlled-pilot recovery posture | `docs/plans/controlled-pilot-recovery-posture-v1.md` | Existing restore evidence, target availability, and recovery claim | Pilot continue/pause decision plus RPO/RTO caveat | Restore target is not disposable, production restore/PITR is required, or secrets would be recorded. |
...
| Choose verification evidence command | `docs/plans/evidence-automation-index-v1.md` | Claim to prove, environment class, and available tokens/provider/target inputs | Command-to-claim mapping plus explicit blockers for missing protected/provider/restore input | A local/mock command would be treated as protected, provider, restore, or production proof. |
