...
| DG1-DG2 locked remediation | evidence-gated/blocked_external | Bounded DG1/DG2 implementation and TARGET DB-C5 staging enforcement are complete. DG1-C remains usage-gated; ORG/ASSIGN remain blocked on exact historical authority; production remains excluded. | `docs/plans/dg1-dg2-locked-decisions-implementation-plan-v1.md`, `docs/plans/post-dg2-staging-remediation-and-constraint-reentry-plan-v1.md` | Verified DG1-C provider usage arrives, exact ORG/ASSIGN historical authority arrives, or a newer owner decision changes the locked boundaries. |
...
| Alert provider posture | active/staging_verified | Better Stack and Render remain health/deploy signals; Sentry Developer app-level delivery is verified in staging for API, worker, and frontend with sanitized receipts. Production activation/escalation ownership remains gated. | `docs/plans/controlled-pilot-alert-incident-policy-v1.md`, `docs/runbooks/dg4-sentry-app-error-delivery-v1.md` | Configuration/release changes invalidate the receipts, or production alert destination/escalation ownership is approved. |
...
| P0 trust operations | active/staging_verified | Sentry delivery remains feature-gated with structured-log fallback; API, worker, and frontend staging receipts are verified. Broad-production trust claims still wait for production alert ownership and the remaining ops gates. | `docs/plans/p0-trust-operations-execution-v1.md`, `docs/plans/dg4-sentry-app-error-delivery-spec-v1.md`, `docs/runbooks/dg4-sentry-app-error-delivery-v1.md` | Configuration/release changes invalidate staging evidence, production ownership is approved, or broad production is requested. |
...
| Project-analysis implementation | active/evidence-gated | A1 (#917), A2 (#918), A3 (#919), B1 blocker recording (#920), E1 (#921), and bounded D1 (#923) are implemented. A3's dated ten-run p95 is `13.23` minutes: the 12-minute target is not met, so two workers and full coverage remain fixed without a new concurrency experiment. B1 retains partial owner-attested mutation evidence and open read-only pilot gaps. | `docs/plans/project-analysis-implementation-plan-v1.md`, `docs/evidence/performance/2026-07-10-e2e-worker-concurrency-a3.md`, `docs/evidence/pilot-readiness/2026-07-05-owner-attested-mutation-flows.md`, `docs/evidence/pilot-readiness/2026-07-10-controlled-pilot-b1-external-blockers.md` | Remaining B1 read-only inputs arrive, a worker-related failure or new measured isolation plan appears, a factual P0/P1 finding appears, or the owner amends the plan. |
| Next runtime train | blocked_external | Open no additional post-DG2 database/runtime PR from the completed TARGET slice. Wait for exact DG1-C usage, ORG/ASSIGN authority, a separately approved target-reference history slice, or factual pilot P0/P1 evidence. | `current-state.md`, `docs/plans/active-next-actions.md` | One named external input arrives or a newer explicit owner instruction opens a bounded runtime slice. |
