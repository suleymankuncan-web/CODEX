...
| DG1-DG2 locked remediation | active | Report Viewer receives the explicit company-scoped Store read allowlist with no action widening; store-team incentives close after required consumer classification; non-development Admin Session becomes read-only; real staging data is preserved while the Norm Kadro demo percentage projection is removed. Database constraints remain read-only-evidence gated. | `docs/plans/dg1-dg2-locked-decisions-implementation-plan-v1.md` | A newer owner decision changes the allowlist/incentive policy, or approved staging evidence resolves the conditional constraint gate. |
...
| Next runtime train | active | Execute the locked DG1-DG2 plan one bounded slice at a time; keep every unrelated runtime, module, mobile, redesign, and broad-production line parked. | `current-state.md`, `docs/plans/dg1-dg2-locked-decisions-implementation-plan-v1.md` | The train completes/blocks on its named evidence, or a newer explicit owner instruction changes direction. |
