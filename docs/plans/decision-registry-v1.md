...
| Store Action V1B | active | Persisted Store Action command path is proven for controlled pilot manager scope; checklist acknowledgement/remediation and target projection are separate locked decisions, both pending implementation slices. | `docs/domains/store-action.md` | Non-KPI sources beyond locked checklist remediation/target projection, raw KPI drift tasks, detail/comments/attachments/escalation, region-manager verification, or broader roles are proposed. |
