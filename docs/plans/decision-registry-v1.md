...
| Alert provider posture | active/blocked_external | Better Stack external health email alert proof plus Render platform notifications remain the health/deploy signals; Sentry Developer is the owner-selected free app-level exception provider with staging receipt pending. | `docs/plans/controlled-pilot-alert-incident-policy-v1.md`, `docs/runbooks/dg4-sentry-app-error-delivery-v1.md` | Sanitized staging receipt, production alert destination/escalation owner, or production profile proof changes the posture. |
...
| P0 trust operations | active/blocked_external | Controlled pilot keeps log-only as fallback while the owner-selected Sentry Developer adapter is implemented behind env flags; broad-production trust claims still wait for sanitized receipt and the remaining ops gates. | `docs/plans/p0-trust-operations-execution-v1.md`, `docs/plans/dg4-sentry-app-error-delivery-spec-v1.md` | Provider/env/redaction evidence is accepted, expires after a configuration change, or broad production is requested. |
