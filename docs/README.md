...
| Continue the post-DG2 staging findings | `docs/plans/post-dg2-staging-remediation-and-constraint-reentry-plan-v1.md`, `docs/plans/rem-2-owner-decision-packet-v1.md`, and `docs/plans/staging-remediation-invariant-spec-v2.md` | REM-1B evidence and all REM-2 owner decisions are locked; PR #955 merged V2 revised semantics and the exact V1 bridge. A fail-closed CA path/content attempt is superseded only through the versioned evidence launcher. Standing authorization removes repeated prompts but does not waive target, evidence, manifest, restore, rollback, concurrency, or production boundaries. |
