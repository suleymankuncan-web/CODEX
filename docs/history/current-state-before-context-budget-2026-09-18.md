# Historical handoff before context budgeting

Status: historical
Shelf: historical
Superseded by: `current-state.md`
Captured from merged source 2db36bd1 on 18 September 2026.

This immutable snapshot preserves prior evidence and historical contract assertions.
It contains superseded statements (including the closed first PDF PR and old
on-prem branch status). Do not use it as current execution or activation authority.
...
- Turkey ranking HG is implemented, pending PR/deployment: selected-day sales divided by full monthly targets for stores and personnel; live HG points equal that ratio times the published admin weight (100% at weight 40 earns 40). Other KPI scales and closed snapshots stay unchanged. Publishing invalidates client rankings; backend profiles/targets remain fresh outside Redis facts. Latest approved store targets and employee/store monthly references apply, each touched month counted once; missing/ambiguous targets leave HG unavailable. Default remains monthly. Scope, tests and rollback: `docs/plans/rankings-monthly-achievement-v1.md`.
- Turkey rankings have an opt-in daily-facts Redis cache (`RANKING_FACTS_CACHE_ENABLED=true`, default off). Local 800-personnel evidence shows a monthly read-path median of 101.86 ms to 45.22 ms and eight-concurrent median of 345.46 ms to 185.19 ms; this is not hosted HTTP/browser proof. Read-only MVCC validation invalidates conservatively on database writes, with an in-statement race fallback; names, permissions, targets and scoring remain fresh. No migration or HG formula change is included. See `docs/plans/rankings-cache-measurement-v1.md` and `docs/evidence/performance/2026-09-16-rankings-cache.md`; hosted activation remains unverified.
- Checklist results show item comments and export styled PDFs with dates, record text and manager signature spaces. The regional manager name follows active roles and direct store assignments at document time, without requiring a linked employee: the account name is used when the employee name is unavailable. History actors use the same account-name fallback. PDFs carry a LUFIAN heading; unavailable names remain explicit. The store manager name is left blank for handwriting. Legacy completion history can resolve the instance's auditor employee. Completed instances retain their template version, item order, weights and stored score; new inspections use their own updated template. Mobile note fields retain readable text sizing, and outside interaction does not start session dismissal; explicit close still requires confirmation. Scoring, permissions and acknowledgement commands are unchanged.
...
- New modules, broad redesign, generic architecture/refactor, and general provider/JSON runtime integration remain parked. The owner-approved on-prem synthetic preparation, contract-only company daily KPI pull specification, and the separately authorized private-server company schedule are bounded local exceptions; the schedule grants no hosted, reusable-connector, or broad-production authority.
- Power BI/Excel remains the current operating data path; the product owner approved the documented source semantics in `docs/contracts/company-daily-kpi-pull-contract-v1.md` on 31 August 2026, the storage/normalization boundary in `docs/contracts/company-daily-kpi-storage-normalization-boundary-v1.md` on 1 September 2026, and the connector readiness evidence contract on 1 September 2026. The network-free pure adapter, typed daily component storage, and atomic replacement repository are present without projecting into canonical KPI scoring. The exact store-range reducer and its bounded internal single-snapshot range-read service are also present without web exposure, connector runtime, or canonical KPI scoring. A pure daily-closure evaluator plus an internal metadata-only persisted daily-closure read service report completed, failed, missed, and absent required components without exposing a controller or recomputing physical facts. A pure fail-closed connector-readiness validator/evaluator is present for synthetic or separately approved sanitized metadata; it has no I/O or runtime wiring and cannot imply source activation. The connector readiness evidence contract is approved; it defines evidence collection requirements but contains no evidence instance. Its public path is `docs/contracts/company-daily-kpi-connector-readiness-evidence-v1.md`.
  Reusable live connector mapping, hosted scheduling, and Excel replacement remain suspended. The private-server schedule and canonical KPI projection described above are active only for the approved company deployment. Observed private evidence and provider-native configuration remain outside Git.
- Runtime work remains parked except for the owner-opened DG1-DG2 remediation train; any other runtime line still requires a factual P0/P1 finding or a newer explicit owner decision.
