...
| `/admin/reports/workforce/:snapshotRunId` | `ReportsWorkforcePage.tsx` | `kpi-config-versioning` mobile guard | 3.8 | Mobile table copy/actions now stack cleanly; dense row cards remain expected for report detail. | Park unless a new browser or pilot gap appears. |
| `/admin/reports/kpis/:snapshotRunId` | `ReportsKpisPage.tsx` | `kpi-config-versioning` + browser family check | 3.8 | Mobile table copy/actions now share the reports detail table treatment. | Park unless a new browser or pilot gap appears. |
| `/admin/reports/checklists/:snapshotRunId` | `ReportsChecklistsPage.tsx` | `kpi-config-versioning` + browser family check | 3.8 | Mobile table copy/actions now share the reports detail table treatment. | Park unless a new browser or pilot gap appears. |
| `/admin/reports/turnover/:snapshotRunId` | `ReportsTurnoverPage.tsx` | `kpi-config-versioning` + browser family check | 3.8 | Mobile table copy/actions now share the reports detail table treatment. | Park unless a new browser or pilot gap appears. |
