...
| Provider | Alert delivery, Redis/BullMQ, Supabase restore, upload provider path, Vercel/Render deployment evidence. | Mixed but no longer one generic blocker. Redis/BullMQ is live in staging health; Render Slack notification delivery, Supabase local logical restore, and current controlled-pilot safe upload proof are recorded. Broad-production Redis tier, final alert policy, and managed restore/PITR posture still need explicit owner decisions. | Use provider-specific runbooks and redact destinations/secrets. |
| Blocked | Evidence that cannot be produced from the current shell without user/provider input. | Current blockers are final alert policy/email or app-level provider proof if required, an explicitly disposable managed Supabase restore target if required, and any future upload rerun that changes role/sample requirements. The Clerk five-persona matrix and current controlled-pilot upload proof are no longer blocked. | Record as blocker; do not fake. |
