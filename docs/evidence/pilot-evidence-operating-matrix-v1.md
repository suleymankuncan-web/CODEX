...
| Protected staging | Clerk/persona route visibility, backend endpoint access, read scope, and assigned-store action scope. | Fresh real Clerk proof exists from 2026-05-23 for the active controlled-pilot persona set: `SUPER_ADMIN`, `HR_ADMIN`, `REGION_MANAGER`, `STORE_MANAGER`, `STORE_PERSONNEL`, and `REPORT_VIEWER`. | Use the persona runbook with sanitized token handling. Never write raw bearer tokens, cookies, JWTs, auth codes, provider subject IDs, or private PII. |
| Provider | Alert delivery, Redis/BullMQ, Supabase restore, upload provider path, Vercel/Render deployment evidence. | Mixed but no longer one generic blocker. Redis/BullMQ is live in staging health; the Redis posture decision accepts Free tier only for controlled pilot and requires persistent Redis or explicit risk acceptance before broad production. Render Slack notification delivery, Better Stack external email test alert delivery, Supabase local logical restore plus controlled-pilot recovery posture, and current controlled-pilot safe upload proof are recorded. App-level error tracking policy and managed restore/PITR posture still need explicit owner decisions for broad production. | Use provider-specific runbooks and redact destinations/secrets. |
| Blocked | Evidence that cannot be produced from the current shell without user/provider input. | Current blockers are production-grade Redis upgrade/evidence before broad production, app-level error tracking if required for broad production, an explicitly disposable managed Supabase restore target if managed/PITR proof is required, and any future upload rerun that changes role/sample requirements. The active Clerk persona matrix, controlled-pilot external email alert proof, Redis controlled-pilot posture, Supabase controlled-pilot recovery posture, and current controlled-pilot upload proof are no longer blocked. | Record as blocker; do not fake. |
...
- Region-manager addendum browser/session smoke passed for `REGION_MANAGER`.
...
| --- | --- | --- |
| Fresh role-specific Clerk bearer/session tokens | Needed to prove protected route visibility, backend endpoint authorization, and assigned-store scoping after the latest deploy. | Closed for the active controlled-pilot persona set on 2026-05-23, including the `REGION_MANAGER` addendum. |
...
| Disposable Supabase restore target | Needed to prove managed restore without touching production/staging data. | Controlled-pilot logical restore posture is accepted on 2026-05-23. Broad production remains blocked until managed/PITR/RPO/RTO posture is explicitly accepted or tested against an approved disposable target. |
