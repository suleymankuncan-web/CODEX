...
| `frontend-unreachable` | Frontend root, SPA fallback, or static asset checks fail. | P0/P1 | Frontend owner + Release operator | Run `npm.cmd run smoke:deployed-readiness`; verify the Cloudflare Worker version, SPA fallback, and asset content types. | Frontend URL, Cloudflare Worker version id, failed asset URL, deployed smoke output. |
