...
test('tiered proof workflow requires mode, image scope, and exact expected SHA inputs', () => {
  ...
  assert.match(sourceGate, /node --check scripts\/onprem-image-local-proof\.mjs/)
  assert.match(sourceGate, /node --check scripts\/onprem-image-local-proof-recovery\.mjs/)
  assert.match(sourceGate, /node --check scripts\/onprem-native-docker-host\.mjs/)
  assert.match(sourceGate, /node --check scripts\/onprem-native-docker-host-provision\.mjs/)
  assert.match(sourceGate, /node --check scripts\/onprem-offline-target-proof\.mjs/)
  assert.match(sourceGate, /onprem-image-local-proof-recovery-contract\.test\.mjs/)
  assert.match(sourceGate, /onprem-native-docker-host-contract\.test\.mjs/)
  assert.match(sourceGate, /onprem-native-docker-host-provision-contract\.test\.mjs/)
