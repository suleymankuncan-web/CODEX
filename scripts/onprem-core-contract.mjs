...
export function validateOnpremCoreContract(input) {
  ...
  const coreRuntimeCommand = joinShellContinuations(coreProofLines).find((command) => command.startsWith('sudo "$(command -v node)" scripts/onprem-core-runtime-proof.mjs --execute'))
  fail(Boolean(coreRuntimeCommand && /--temporary-directory "\$RUNNER_TEMP"/.test(coreRuntimeCommand)), 'required CI runtime proof must keep root-run temporary TLS material inside RUNNER_TEMP')
