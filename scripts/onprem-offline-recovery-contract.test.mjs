...
test('ONP-5 operators encode the signed synthetic rehearsal boundaries', () => {
  ...
  assert.match(restore, /REASSIGN OWNED BY hr_axis_bootstrap TO hr_axis_migrator/)
  assert.match(restore, /REASSIGN OWNED BY hr_axis_bootstrap TO keycloak/)
  assert.match(restore, /repair_database_ownership hr-axis/)
  assert.match(restore, /repair_database_ownership keycloak/)
  assert.match(restore, /GRANT hr_axis_migrator TO hr_axis_bootstrap/)
  assert.match(restore, /GRANT keycloak TO hr_axis_bootstrap/)
  assert.match(restore, /REVOKE hr_axis_migrator FROM hr_axis_bootstrap/)
  assert.match(restore, /REVOKE keycloak FROM hr_axis_bootstrap/)
  ...
  assert.match(restore, /repair_sql='GRANT hr_axis_migrator TO hr_axis_bootstrap;[\s\S]*REASSIGN OWNED BY hr_axis_bootstrap TO hr_axis_migrator;'/)
  assert.match(restore, /repair_sql='GRANT keycloak TO hr_axis_bootstrap;[\s\S]*REASSIGN OWNED BY hr_axis_bootstrap TO keycloak;'/)
