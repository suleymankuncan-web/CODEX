...
test('ONP-5 operators encode the signed synthetic rehearsal boundaries', () => {
  ...
  assert.match(restore, /--set=ON_ERROR_STOP=1[\s\S]*--dbname=hr_axis[\s\S]*REASSIGN OWNED BY hr_axis_bootstrap TO hr_axis_migrator/)
  assert.match(restore, /--set=ON_ERROR_STOP=1[\s\S]*--dbname=keycloak[\s\S]*REASSIGN OWNED BY hr_axis_bootstrap TO keycloak/)
