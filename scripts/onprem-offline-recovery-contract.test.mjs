...
test('ONP-5 operators encode the signed synthetic rehearsal boundaries', () => {
  ...
  assert.match(upgrade, /timeout --signal=TERM --kill-after="\$\{TARGET_RESTORE_KILL_AFTER_SECONDS\}s" "\$\{TARGET_RESTORE_TIMEOUT_SECONDS\}s"/)
  ...
  assert.match(rollback, /timeout --signal=TERM --kill-after="\$\{TARGET_RESTORE_KILL_AFTER_SECONDS\}s" "\$\{TARGET_RESTORE_TIMEOUT_SECONDS\}s"/)
