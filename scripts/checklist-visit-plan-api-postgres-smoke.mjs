...
const database = process.env.MIGRATION_SMOKE_DB_NAME ?? "store_ops_fresh_migration_smoke_visit_plan_api";

if (process.env.NODE_ENV === "production") fail("Visit plan API smoke is local-only");
if (!/^store_ops_fresh_migration_smoke(_[a-z0-9_]+)?$/.test(database)) fail("Unsafe disposable database name");
if (!["localhost", "127.0.0.1"].includes(host)) fail("Visit plan API smoke requires localhost");

runNpm(workspaceRoot, ["run", "smoke:migration:fresh-db"], {
  MIGRATION_SMOKE_DB_NAME: database,
});
runNpm(backendRoot, ["exec", "ts-node", "scripts/checklist-visit-plan-api-postgres-smoke.ts"], {
