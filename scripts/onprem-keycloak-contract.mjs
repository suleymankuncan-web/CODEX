...
export function validateOnpremKeycloakContract(input) {
  ...
  fail(/readonly KCADM_TIMEOUT_SECONDS=20/.test(input.bootstrapScript)
  ...
  fail(/credentials_ready=false/.test(input.bootstrapScript)
    && /while \[ "\$attempt" -lt 30 \]; do/.test(input.bootstrapScript)
  ...
  fail(/readonly KCADM_TIMEOUT_SECONDS=20/.test(input.bootstrapScript)
    && /readonly BOOTSTRAP_TIMEOUT_SECONDS=900/.test(input.bootstrapScript)
    ...
    && /while \[ "\$attempt" -lt 30 \]; do/.test(input.bootstrapScript), 'bootstrap must enforce bounded per-request and whole-reconciliation timeouts')
