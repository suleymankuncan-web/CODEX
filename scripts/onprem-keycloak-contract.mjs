...
export function validateOnpremKeycloakContract(input) {
  ...
  fail(/cat\s+"\$bootstrap_password_file"\s*\|[\s\S]*kcadm\.sh config credentials/.test(input.bootstrapScript), 'kcadm service credential login must read its secret from stdin')
