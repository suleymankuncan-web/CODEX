...
test('offline rehearsal downloads only the bundle, cuts egress before verification, and never checks out source', () => {
  ...
  assert.match(rehearsal, /"\$SETSID_BIN" sudo env "PATH=\$OPERATOR_PATH" timeout --signal=TERM --kill-after="\$\{kill_after_seconds\}s" "\$\{timeout_seconds\}s"/)
  assert.match(rehearsal, /kill -TERM -- "-\$pid"/)
  assert.match(rehearsal, /kill -KILL -- "-\$pid"/)
