...
test('offline rehearsal enforces Docker and host IPv4/IPv6 egress with bound negative probes', () => {
  ...
  assert.ok(rehearsal.includes('host_dns_uid="$(id -u systemd-resolved 2>/dev/null || id -u systemd-resolve 2>/dev/null || true)"'))
  assert.match(rehearsal, /test -n "\$host_dns_uid" && test "\$host_dns_uid" -gt 0/)
  ...
  assert.match(rehearsal, /iptables -A "\$host_egress_chain" -m owner --uid-owner "\$host_dns_uid" -p udp --dport 53 -j ACCEPT/)
  assert.match(rehearsal, /iptables -A "\$host_egress_chain" -m owner --uid-owner "\$host_dns_uid" -p tcp --dport 53 -j ACCEPT/)
  assert.match(rehearsal, /ip6tables -A "\$host6_egress_chain" -m owner --uid-owner "\$host_dns_uid" -p udp --dport 53 -j ACCEPT/)
  assert.match(rehearsal, /ip6tables -A "\$host6_egress_chain" -m owner --uid-owner "\$host_dns_uid" -p tcp --dport 53 -j ACCEPT/)
  assert.match(rehearsal, /sudo iptables -I OUTPUT 1 -j "\$host_egress_chain"/)
  assert.match(rehearsal, /sudo ip6tables -I OUTPUT 1 -j "\$host6_egress_chain"/)
  const host4RuleCommands = rehearsal.match(/sudo iptables -A "\$host_egress_chain"/g) ?? []
  const host6RuleCommands = rehearsal.match(/sudo ip6tables -A "\$host6_egress_chain"/g) ?? []
  const host4RuleCountGuard = rehearsal.match(/test "\$\(sudo iptables -S "\$host_egress_chain" \| wc -l\)" -eq ([0-9]+)/)
  const host6RuleCountGuard = rehearsal.match(/test "\$\(sudo ip6tables -S "\$host6_egress_chain" \| wc -l\)" -eq ([0-9]+)/)
  assert.equal(host4RuleCommands.length, 6)
  assert.equal(host6RuleCommands.length, 6)
  assert.ok(host4RuleCountGuard)
  assert.ok(host6RuleCountGuard)
  assert.equal(Number(host4RuleCountGuard[1]), host4RuleCommands.length + 1)
  assert.equal(Number(host6RuleCountGuard[1]), host6RuleCommands.length + 1)
