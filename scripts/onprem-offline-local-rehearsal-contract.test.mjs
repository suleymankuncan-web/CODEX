...
function supervisedFixture({ phaseFailure = false, resetFailure = false, resetUnverified = false, ipv6Mismatch = false, ipv6CounterOnlyMismatch = false, ipv6DiagnosticOverflow = false, semanticTimestampDrift = false, emptyAutoRaw = false, postInventory = null, finalGit = null } = {}) {
  const directory = temporaryDirectory()
  ...
  const emptyRaw = (family, timestamp) => Buffer.from([
    `# Generated by ${family} v1.8.9 (nf_tables) on ${timestamp}`,
    '*raw', ':PREROUTING ACCEPT [0:0]', ':OUTPUT ACCEPT [0:0]', 'COMMIT', `# Completed on ${timestamp}`,
  ].join('\n') + '\n', 'ascii')
  const filterBefore = semanticFirewall('iptables-save', 'Mon Jun 10 09:18:34 2024')
  const filterAfter = semanticFirewall('iptables-save', 'Tue Jun 11 10:19:35 2024')
  const natBefore = Buffer.from(filterBefore.toString('ascii').replace('*filter', '*nat'), 'ascii')
  const natAfter = Buffer.from(filterAfter.toString('ascii').replace('*filter', '*nat'), 'ascii')
  const ipv4Bytes = emptyAutoRaw ? Buffer.concat([filterBefore, natBefore]) : semanticTimestampDrift ? filterBefore : Buffer.from('ipv4-before\n')
  const ipv4AfterBytes = emptyAutoRaw ? Buffer.concat([emptyRaw('iptables-save', 'Tue Jun 11 10:19:35 2024'), filterAfter, natAfter]) : semanticTimestampDrift ? filterAfter : ipv4Bytes
...
test('offline recovery records the narrow empty automatic raw-table allowance', () => {
  const fixture = supervisedFixture({ emptyAutoRaw: true })
  try {
    const result = runSupervisedLifecycle(fixture.context, { git: { head: fixture.options.sourceSha, tree: fixture.options.treeSha }, inputIdentity: { manifest: { sourceRevision: fixture.options.sourceSha } } }, {
      lock: { path: 'lock-held' }, beforeHost: fixture.beforeHost, snapshots: fixture.snapshots, commandRunner: fixture.commandRunner, platform: 'linux', uid: 1000,
      controller: fixture.controller, execute: fixture.execute, finalGit: { head: fixture.options.sourceSha, tree: fixture.options.treeSha },
    ...
    assert.equal(result.recovery.firewall.timestampOnlyEquivalent, false)
    assert.equal(result.recovery.firewall.emptyAutoRawTableEquivalent, true)
    assert.equal(result.recovery.firewall.equivalent, true)
    assert.equal(result.recovery.firewall.ipv4.emptyAutoRawTableEquivalent, true)
    assert.equal(result.recovery.firewall.ipv4.timestampOnlyEquivalent, false)
    assert.equal(result.recovery.firewall.ipv6.emptyAutoRawTableEquivalent, false)
    assert.equal(result.receipt.recovery.firewall.emptyAutoRawTableEquivalent, true)
    assert.equal(result.receipt.recovery.firewall.ipv4.emptyAutoRawTableEquivalent, true)
  } finally {
    fs.rmSync(fixture.directory, { recursive: true, force: true })
  }
})
