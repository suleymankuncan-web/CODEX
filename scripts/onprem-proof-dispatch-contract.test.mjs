...
test('Linux proof Docker command is source-archive-only, pinned, offline, read-only, and capability bounded', () => {
  ...
  assert.ok(args.includes('--user') && args[args.indexOf('--user') + 1] === '1000:1000')
  ...
  assert.ok(args.some((value) => value === '/workspace:rw,nosuid,nodev,size=512m,mode=0755,uid=1000,gid=1000'))
  assert.ok(args.some((value) => value === '/tmp:rw,nosuid,nodev,size=512m,mode=0700,uid=1000,gid=1000'))
  assert.ok(args.every((value) => !value.includes('mode=1777')))
  assert.ok(args.some((value) => value === '/test-tmp:rw,nosuid,nodev,exec,size=512m,mode=0700,uid=1000,gid=1000'))
  assert.deepEqual(args.slice(args.indexOf('--env'), args.indexOf('--env') + 2), ['--env', 'TMPDIR=/test-tmp'])
  assert.ok(args.some((value) => value === '/var/lib:rw,nosuid,nodev,size=512m,mode=0755,uid=1000,gid=1000'))
