...
test('timestamp-only comparator is raw-first, family-bound, and strict about repeated documents', () => {
  ...
  assert.deepEqual(binary, { byteEqual: true, timestampOnlyEquivalent: false, equivalent: true, lineDigestTruncated: false })
  ...
  for (const [before, after, family] of [[ipv4Before, ipv4After, 'iptables-save'], [ipv6Before, ipv6After, 'ip6tables-save']]) {
    assert.deepEqual(compareFirewallSnapshots(before, after, family), { byteEqual: false, timestampOnlyEquivalent: true, equivalent: true, lineDigestTruncated: false })
  }
