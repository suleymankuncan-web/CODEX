...
export function AuthManagementPage() {
  ...
  return (
    <AdminSurfacePage ariaLabel="Erişim yönetimi" className="tw:max-w-none tw:gap-5 tw:font-sans">
            ...
            <div aria-label="Kullanıcı listesi" className="tw:max-h-[640px] tw:overflow-y-auto tw:px-3 tw:py-2">
              {(usersQuery.data?.items ?? []).map((user) => (
                ...
                >
                  ...
                  <span className={`tw:shrink-0 tw:rounded-full tw:px-2 tw:py-1 tw:text-[10px] tw:font-semibold ${user.isActive ? 'tw:bg-emerald-50 tw:text-emerald-700' : 'tw:bg-slate-100 tw:text-slate-500'}`}>{user.isActive ? 'Aktif' : 'Pasif'}</span>
                </button>
              ))}
            </div>
          ...
          {selectedUser ? (
            <section className="tw:min-w-0 tw:overflow-hidden tw:rounded-[24px] tw:border tw:border-border tw:bg-card tw:shadow-[0_20px_50px_-42px_rgba(15,30,68,0.7)]">
              <header className="tw:bg-white tw:px-5 tw:py-5 tw:sm:px-7 tw:sm:py-6">
                <div className="tw:flex tw:flex-col tw:gap-5 tw:sm:flex-row tw:sm:items-start tw:sm:justify-between">
                  <div className="tw:flex tw:min-w-0 tw:items-center tw:gap-4">
                    ...
                    <div className="tw:min-w-0"><div className="tw:flex tw:flex-wrap tw:items-center tw:gap-2"><h2 className="tw:m-0 tw:truncate tw:text-xl tw:font-semibold tw:tracking-[-0.025em]">{selectedUser.username}</h2><Badge className={selectedUser.isActive ? 'tw:border-emerald-200 tw:bg-emerald-50 tw:text-emerald-800' : 'tw:border-slate-200 tw:bg-slate-100 tw:text-slate-600'} variant="outline">{selectedUser.isActive ? 'Aktif hesap' : 'Pasif hesap'}</Badge></div><p className="tw:mt-1 tw:mb-0 tw:flex tw:items-center tw:gap-1.5 tw:truncate tw:text-sm tw:text-muted-foreground"><Mail className="tw:size-3.5" aria-hidden="true" />{selectedUser.email}</p></div>
                  </div>
                  <Button className="tw:self-start" onClick={() => setAccountAction(selectedUser)} size="sm" variant="outline">{selectedUser.isActive ? 'Devre dışı bırak' : 'Yeniden etkinleştir'}</Button>
                </div>
                <div className="tw:mt-6 tw:flex tw:flex-wrap tw:gap-x-7 tw:gap-y-2 tw:border-t tw:border-border tw:pt-4 tw:text-xs tw:text-muted-foreground">
                  <span><strong className="tw:mr-1 tw:text-foreground">{roleAssignmentsQuery.data?.items.length ?? 0}</strong> aktif rol</span>
                  <span><strong className="tw:mr-1 tw:text-foreground">{storeAssignmentsQuery.data?.items.length ?? 0}</strong> doğrudan mağaza</span>
              ...
              <div className="tw:grid tw:gap-4 tw:p-4 tw:sm:p-5 tw:xl:grid-cols-2">
                <AccessBlock icon={<Shield aria-hidden="true" />} title="Roller" action={<Button onClick={() => setRoleOpen(true)} size="sm" variant="outline"><Plus aria-hidden="true" /> Rol ekle</Button>}>
                  {(roleAssignmentsQuery.data?.items ?? []).length ? roleAssignmentsQuery.data?.items.map((assignment) => (
                    <AccessRow key={assignment.assignmentId} label={roleDisplayName(assignment)} meta={scopeLabel(assignment)} onRemove={() => removeRoleMutation.mutate(assignment.assignmentId)} />
                  )) : <EmptyAccess copy="Rol ataması yok" />}
                </AccessBlock>
                <AccessBlock icon={<Building2 aria-hidden="true" />} title="Mağaza erişimi" action={<Button onClick={() => setStoreOpen(true)} size="sm" variant="outline"><Plus aria-hidden="true" /> Mağaza ekle</Button>}>
                  {(storeAssignmentsQuery.data?.items ?? []).length ? storeAssignmentsQuery.data?.items.map((assignment) => (
      ...
      <RoleAssignmentDialog open={roleOpen} onOpenChange={setRoleOpen} roles={lookupsQuery.data?.roles ?? []} stores={lookupsQuery.data?.stores ?? []} user={selectedUser} onSave={(draft) => createRoleMutation.mutate(draft)} pending={createRoleMutation.isPending} />
      <StoreAssignmentDialog key={`${selectedUser?.userId ?? 'none'}-${storeOpen ? 'open' : 'closed'}`} assignedStoreIds={(storeAssignmentsQuery.data?.items ?? []).map((item) => item.storeId)} open={storeOpen} onOpenChange={setStoreOpen} stores={lookupsQuery.data?.stores ?? []} user={selectedUser} onSave={(draft) => createStoreMutation.mutate(draft)} pending={createStoreMutation.isPending} />
...
function CreateUserDialog({ open, onOpenChange, onSave, pending }: { open: boolean; onOpenChange: (open: boolean) => void; onSave: (draft: { username: string; email: string; authProvider: 'local' | 'oidc' | 'sso' | 'clerk'; providerSubject?: string }) => void; pending: boolean }) {
  const [draft, setDraft] = useState({ username: '', email: '', authProvider: 'clerk' as const, providerSubject: '' })
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent closeLabel="Kapat"><DialogHeader><DialogTitle>Kullanıcı ekle</DialogTitle><DialogDescription>Kimlik sağlayıcısındaki hesapla eşleşecek yönetim hesabını oluşturun.</DialogDescription></DialogHeader><div className="tw:grid tw:gap-3"><Field label="Kullanıcı adı"><Input aria-label="Kullanıcı adı" value={draft.username} onChange={(e) => setDraft({ ...draft, username: e.target.value })} /></Field><Field label="E-posta"><Input aria-label="E-posta" type="email" value={draft.email} onChange={(e) => setDraft({ ...draft, email: e.target.value })} /></Field><Field label="Kimlik sağlayıcı"><Select value={draft.authProvider} onValueChange={(value) => setDraft({ ...draft, authProvider: value as typeof draft.authProvider })}><SelectTrigger aria-label="Kimlik sağlayıcı" className="tw:w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="clerk">Clerk</SelectItem><SelectItem value="oidc">OIDC</SelectItem><SelectItem value="sso">SSO</SelectItem><SelectItem value="local">Yerel</SelectItem></SelectContent></Select></Field><Field label="Sağlayıcı kullanıcı kimliği"><Input aria-label="Sağlayıcı kullanıcı kimliği" value={draft.providerSubject} onChange={(e) => setDraft({ ...draft, providerSubject: e.target.value })} placeholder="Örn. user_..." /></Field></div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Vazgeç</Button><Button disabled={draft.username.trim().length < 3 || !draft.email.includes('@') || pending} onClick={() => onSave({ username: draft.username, email: draft.email, authProvider: draft.authProvider, ...(draft.providerSubject.trim() ? { providerSubject: draft.providerSubject.trim() } : {}) })}><Plus aria-hidden="true" /> Oluştur</Button></DialogFooter></DialogContent></Dialog>
}

function RoleAssignmentDialog({ open, onOpenChange, roles, stores, user, onSave, pending }: { open: boolean; onOpenChange: (open: boolean) => void; roles: Array<{ roleCode: string; roleName: string; scopeType: string }>; stores: Array<{ storeId: string; storeName: string; companyId: string; regionId: string; regionName: string }>; user: UserAccount | null; onSave: (draft: { userId: string; roleCode: 'REGION_MANAGER' | 'STORE_MANAGER' | 'VISUAL_MERCHANDISER' | 'SUPER_ADMIN' | 'HR_ADMIN' | 'INTEGRATION_ADMIN' | 'SNAPSHOT_OPERATOR' | 'REPORT_VIEWER' | 'AUDITOR' | 'STORE_PERSONNEL'; scopeType: 'company' | 'region' | 'store'; companyId?: string; regionId?: string; storeId?: string }) => void; pending: boolean }) {
  const [roleCode, setRoleCode] = useState('')
  const selectedRole = roles.find((role) => role.roleCode === roleCode)
  const scopeType = (selectedRole?.scopeType ?? 'company') as 'company' | 'region' | 'store'
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent closeLabel="Kapat"><DialogHeader><DialogTitle>Rol ekle</DialogTitle><DialogDescription>{user?.username} için rol ve kapsam seçin.</DialogDescription></DialogHeader><div className="tw:grid tw:gap-3"><Field label="Rol"><Select value={roleCode} onValueChange={(value) => { setRoleCode(value); setScopeId('') }}><SelectTrigger aria-label="Rol" className="tw:w-full"><SelectValue placeholder="Rol seçin" /></SelectTrigger><SelectContent>{roles.map((role) => <SelectItem key={role.roleCode} value={role.roleCode}>{roleDisplayName(role)}</SelectItem>)}</SelectContent></Select></Field>{scopeType === 'region' ? <Field label="Bölge"><Select value={scopeId} onValueChange={setScopeId}><SelectTrigger aria-label="Rol bölgesi" className="tw:w-full"><SelectValue placeholder="Bölge seçin" /></SelectTrigger><SelectContent>{regions.map((region) => <SelectItem key={region.regionId} value={region.regionId}>{region.regionName}</SelectItem>)}</SelectContent></Select></Field> : null}{scopeType === 'store' ? <Field label="Mağaza"><Select value={scopeId} onValueChange={setScopeId}><SelectTrigger aria-label="Rol mağazası" className="tw:w-full"><SelectValue placeholder="Mağaza seçin" /></SelectTrigger><SelectContent>{stores.map((store) => <SelectItem key={store.storeId} value={store.storeId}>{store.storeName}</SelectItem>)}</SelectContent></Select></Field> : null}</div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Vazgeç</Button><Button disabled={!user || !roleCode || !companyId || (scopeType !== 'company' && !scopeId) || pending} onClick={() => user && companyId && onSave({ userId: user.userId, roleCode: roleCode as Parameters<typeof onSave>[0]['roleCode'], scopeType, companyId, ...(scopeType === 'region' ? { regionId: scopeId } : {}), ...(scopeType === 'store' ? { storeId: scopeId } : {}) })}><ShieldCheck aria-hidden="true" /> Rolü ata</Button></DialogFooter></DialogContent></Dialog>
}

function StoreAssignmentDialog({ assignedStoreIds, open, onOpenChange, stores, user, onSave, pending }: { assignedStoreIds: string[]; open: boolean; onOpenChange: (open: boolean) => void; stores: Array<{ storeId: string; storeName: string; storeCode: string }>; user: UserAccount | null; onSave: (draft: { userId: string; storeIds: string[] }) => void; pending: boolean }) {
  const [query, setQuery] = useState('')
...
function scopeLabel(assignment: { scopeType: string; companyId: string | null; regionId: string | null; storeId: string | null }) { return assignment.scopeType === 'store' ? 'Mağaza kapsamı' : assignment.scopeType === 'region' ? 'Bölge kapsamı' : 'Şirket kapsamı' }
