...
export function RequestCenterDrawer(input: { copy: RequestCenterCopy; row: RequestCenterRow | null; onOpenChange: (open: boolean) => void }) {
  const row = input.row
  return <Sheet open={Boolean(row)} onOpenChange={input.onOpenChange}><CommandCanvasOperationalDrawerContent className="approvals-command-drawer tw:p-0" closeLabel={input.copy.close}>
    {row ? <><SheetHeader className="approvals-command-drawer-header"><div className="tw:pr-8"><RequestStatus row={row} /></div><SheetTitle className="tw:text-xl tw:font-semibold">{row.title}</SheetTitle><SheetDescription>{input.copy.drawerDescription}</SheetDescription></SheetHeader>
      <div className="approvals-command-drawer-body"><div className="tw:grid tw:grid-cols-2 tw:gap-3"><Fact label={input.copy.factsType} value={row.subtitle} /><Fact label={input.copy.factsStore} value={row.scopeTitle} /><Fact label={input.copy.factsWaiting} value={row.waitingLabel} /><Fact label={input.copy.factsOwner} value={row.nextOwnerLabel} /><Fact label={input.copy.factsUpdated} value={row.updatedLabel} /><Fact label={input.copy.factsDue} value={row.dueLabel} /></div>
      ...
      </div><SheetFooter className="approvals-command-drawer-footer"><Button asChild className="tw:min-h-11 tw:rounded-xl"><Link to={row.actionTo}>{row.actionLabel}<ArrowRight className="tw:size-4" /></Link></Button></SheetFooter></> : null}
