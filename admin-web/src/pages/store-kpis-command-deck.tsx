...
const storeMetricOrder = ['TARGET_ACHIEVEMENT', 'UPT', 'ATV', 'CR', 'BM_CHECKLIST', 'VM_CHECKLIST']
...
export function StoreKpisCommandDeck({ model }: { model: StoreKpiHighlightsPageModel }) {
  ...
  const personnelRows = model.viewMode === 'live' && managedPersonnelRows.length > 0
    ? managedPersonnelRows
    : model.viewMode === 'live'
      ? personnelLeaderboard?.items ?? []
      : []
