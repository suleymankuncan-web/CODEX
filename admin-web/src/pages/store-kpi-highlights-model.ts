import { buildStoreKpiLivePeriodControls } from './store-kpis-live-period'
...
export function useStoreKpiHighlightsPageModel(input: { authSummary: AuthSessionSummary | null }) {
  ...
  const {
    ...
    regionOverviewPage, regionOverviewPageSize, regionOverviewSearch,
    regionOverviewSummaryQuery,
    regionOverviewSort,
    reportViewerOverviewQuery,
    reportViewerPage, reportViewerManagerSearch, reportViewerPageSize,
    reportViewerRiskOnly,
    reportViewerRiskPage,
    setRegionOverviewPage, setRegionOverviewSearch,
    setRegionOverviewPeriodStart,
    setRegionOverviewSort,
    setReportViewerPage, setReportViewerManagerSearch,
  ...
  const latestLivePeriodLabel =
    liveSummary?.period
      ? `${formatDate(liveSummary.period.periodStart, locale)} - ${formatDate(liveSummary.period.periodEnd, locale)} (${t(livePeriodType === 'daily' ? 'storeKpis.periodType.daily' : 'storeKpis.latestMonthlyPeriod')})`
      : t(livePeriodType === 'daily' ? 'storeKpis.periodType.daily' : 'storeKpis.latestMonthlyPeriod')
