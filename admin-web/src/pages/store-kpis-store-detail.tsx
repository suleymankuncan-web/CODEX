...
export function StoreKpisStoreDetail({ model }: { model: StoreKpiHighlightsPageModel }) {
  ...
  const personnelRankingQuery = useQuery({
    queryKey: ['store-kpis-personnel-ranking', storeId || 'no-store', periodStart || 'latest-monthly', personnelPage, personnelPageSize, model.livePeriodType, model.kpiDateRangeEnd],
    queryFn: () =>
      getRankings({
    ...
    placeholderData: (previous, query) =>
      ...
      query.queryKey[5] === model.livePeriodType && query.queryKey[6] === model.kpiDateRangeEnd ? previous : undefined,
  })
