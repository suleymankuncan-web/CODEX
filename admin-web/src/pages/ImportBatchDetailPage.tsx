...
import {
  ...
  ErrorRowsPanel,
  useScopedOffset,
} from '../features/integrations/import-batch-detail-activity-panels'
import {
  buildImportDecisionEvidence,
  buildKpiReviewEvidence,
  buildKpiReviewExportRows,
  formatEntityType,
  formatIssueOwner,
  formatQualityIssueDescription,
  formatQualityIssueLabel,
  formatQualitySeverity,
  formatRowCount,
  formatStateLabel,
  type ImportDecisionEvidence,
  type KpiReviewEvidence,
} from '../features/integrations/import-batch-detail-evidence'
import {
  DependencyCard,
  DetailList,
  DetailProgressRow,
...
import { mapQualitySeverityTone, toAdminTone } from '../features/integrations/integration-surface-tone'
...
export function ImportBatchDetailPage() {
  ...
  const [errorOffset, setErrorOffset] = useScopedOffset(batchId)
  const [auditOffset, setAuditOffset] = useScopedOffset(batchId)
  ...
  const errorsQuery = useQuery({
    queryKey: ['import-batch-errors', batchId, ERROR_PAGE_SIZE, errorOffset],
    queryFn: () => getImportBatchErrors(batchId, { limit: ERROR_PAGE_SIZE, offset: errorOffset }),
    enabled: Boolean(batchId),
  })
  const auditQuery = useQuery({
    queryKey: ['import-batch-audit', batchId, AUDIT_PAGE_SIZE, auditOffset],
    queryFn: () => getImportBatchAudit(batchId, { limit: AUDIT_PAGE_SIZE, offset: auditOffset }),
    enabled: Boolean(batchId),
  })
...
function KpiReviewPanel(input: {
  ...
  const canExport = evidence.totalErrorRows > 0
  const handleExport = async () => {
    ...
    try {
      ...
      downloadCsv({
        ...
        rows: buildKpiReviewExportRows(allErrors, t),
      })
    } catch (error) {
      actionToast.error(
        error,
    ...
    <AdminSurfaceSection
      ...
      actions={
        <AdminActionRow className="tw:justify-start tw:sm:justify-end">
          ...
          >
            {isExporting ? t('importBatchDetail.exportPreparing') : t('importBatchDetail.exportReviewRows')}
          </Button>
        </AdminActionRow>
      }
