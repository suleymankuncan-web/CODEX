  ...
  return <Sheet open={input.store !== null} onOpenChange={(open) => { if (!open && !input.pending) input.onClose() }}>
    <CommandCanvasOperationalDrawerContent className="target-command-drawer">
      ...
      {input.store ? <div className="target-command-drawer-body">
        ...
        <section className="target-command-allocations"><div className="target-command-section-heading"><div><h3>{copy.allocation}</h3><p>{input.editable ? copy.editHint : copy.readOnlyHint}</p></div></div>{request.allocations.length ? request.allocations.map((allocation) => { const role = input.store?.personnel.find((person) => person.employeeId === allocation.employeeId)?.positionLabel ?? '—'; const isEditing = editing === allocation.employeeId; return <div className={`target-command-person ${isEditing ? 'is-editing' : ''}`} key={allocation.employeeId}><button disabled={!input.editable} onClick={() => setEditing((current) => current === allocation.employeeId ? null : allocation.employeeId)} type="button"><span><strong>{allocation.displayName}</strong><small>{role}</small></span><span><strong>{formatTargetMoney(drafts[allocation.employeeId], locale)}</strong><small>{target > 0 ? `%${Math.round(Number(drafts[allocation.employeeId] ?? 0) / target * 100)} pay` : '—'}</small></span></button>{isEditing ? <div className="target-command-inline-editor"><Label><span className="tw:sr-only">{allocation.displayName}</span><Input autoFocus aria-label={`${allocation.displayName} hedefi`} inputMode="decimal" value={drafts[allocation.employeeId] ?? ''} onChange={(event) => setDrafts((current) => ({ ...current, [allocation.employeeId]: event.target.value.replace(/[^0-9.,]/g, '').replace(',', '.') }))} /></Label><Button onClick={() => setEditing(null)} size="sm" variant="outline">{copy.close}</Button></div> : null}</div> }) : <div className="target-command-drawer-empty">{copy.noPersonnel}</div>}</section>
