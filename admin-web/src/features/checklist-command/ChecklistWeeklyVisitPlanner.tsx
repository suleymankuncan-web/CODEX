            ...
            <section className="week-plan-picker">
              ...
              <div className="week-plan-results">
                {!candidatesQuery.data && candidatesQuery.isLoading ? <PlannerState label={input.copy.loadingStores} /> : candidatesQuery.isError && !candidatesQuery.data ? <PlannerState label={input.copy.storeLoadFailed} action={input.copy.retry} onAction={() => void candidatesQuery.refetch()} /> : candidates.length === 0 ? <PlannerState label={input.copy.noStores} /> : candidates.map((store) => {
                  const plannedIndex = selectedDay ? drafts.findIndex((item) => item.storeId === store.storeId && item.plannedDate === selectedDay.isoDate) : -1
                  const planned = plannedIndex >= 0
                  return <article className="week-plan-result-row" key={store.storeId}><span><strong>{store.storeName}</strong></span>{planned ? <><button className="week-plan-added-action" type="button" aria-label={input.copy.storeAddedLabel(store.storeName, selectedDay?.dayLabel ?? '')} disabled><Check size={13} />{input.copy.added}</button><button className="week-plan-mobile-remove" type="button" aria-label={input.copy.removeStore(store.storeName)} onClick={() => removeDraft(plannedIndex)}><X size={13} />{input.copy.remove}</button></> : <button type="button" aria-label={input.copy.addStoreLabel(store.storeName, selectedDay?.dayLabel ?? '')} onClick={() => addStore(store.storeId)}><Plus size={13} />{input.copy.add}</button>}</article>
                })}
              </div>
