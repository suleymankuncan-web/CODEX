...
test('store rankings page renders Plum ranking table without signal chrome', async ({ page }) => {
  ...
  await expect(periodFilter).toContainText('Nis 2026')
  await periodFilter.click()
  const calendar = page.getByRole('dialog', { name: 'Dönem seç' })
  await expect(calendar.getByRole('combobox', { name: 'Ay seç' })).toHaveValue('3')
  await expect(calendar.getByRole('combobox', { name: 'Yıl seç' })).toHaveValue('2026')
  await expect(calendar.getByRole('button', { name: 'Tüm ay' })).toBeVisible()
