...
for (const view of ['region_manager', 'report_viewer'] as const) {
  for (const width of [1440, 1024, 915, 390, 320]) {
    test(`${view} list and drawer remain readable at ${width}px`, async ({ page }, testInfo) => {
      ...
      const opener = page.getByLabel('Mall of İstanbul: Prim ayrıntılarını aç').filter({ visible: true })
      await opener.click()
...
test('monthly view shows cumulative daily sales without changing closed incentive amounts', async ({ page }) => {
  ...
  await expect(drawer.getByText('Ay içi net satış')).toBeVisible()
  await expect(drawer.getByText('₺500.000,00')).toBeVisible()
  await expect(drawer.getByText('₺8.721.540,00').first()).toBeVisible()
