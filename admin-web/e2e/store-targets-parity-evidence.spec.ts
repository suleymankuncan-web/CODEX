...
for (const scenario of [
  ...
  test(`Targets ${scenario.view} frame at ${scenario.width}x${scenario.height}`, async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: scenario.view === 'report_viewer' ? 'Şirket hedef görünümü' : 'Hedef Kontrol Masası' })).toBeVisible()
    ...
    if (capture) await page.locator('.store-shell').screenshot({ animations: 'disabled', path: evidencePath(scenario.file) })
  })
}

for (const scenario of [
  { persona: 'regionManager' as const, view: 'region_manager' as const, width: 1440, height: 900, file: 'region-manager-drawer-desktop.png' },
  { persona: 'regionManager' as const, view: 'region_manager' as const, width: 320, height: 844, file: 'region-manager-drawer-narrow.png' },
  { persona: 'reportViewer' as const, view: 'report_viewer' as const, width: 1440, height: 900, file: 'report-viewer-drawer-desktop.png' },
  { persona: 'reportViewer' as const, view: 'report_viewer' as const, width: 320, height: 844, file: 'report-viewer-drawer-narrow.png' },
]) {
  test(`Targets ${scenario.view} drawer at ${scenario.width}x${scenario.height}`, async ({ page }) => {
    ...
    if (scenario.width === 1440) {
      ...
      const cards = await drawer.locator('.target-command-drawer-metrics > div').evaluateAll((elements) => elements.map((element) => ({ x: element.getBoundingClientRect().x, y: element.getBoundingClientRect().y })))
    ...
    if (capture) await page.screenshot({ animations: 'disabled', path: evidencePath(scenario.file) })
  })
}

async function prepare(page: Page, persona: 'regionManager' | 'reportViewer', view: 'region_manager' | 'report_viewer') {
