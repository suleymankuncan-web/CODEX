...
test('store self-performance page renders live score, metrics, and ranks', async ({ page }) => {
  ...
  await expect(page.getByText('Performans ozeti')).toBeVisible()
  await expect(page.getByText('Donem performansi')).toBeVisible()
  await expect(page.getByText('Turkiye siram')).toBeVisible()
  await expect(page.getByText('Magaza ici siram')).toBeVisible()
  await expect(page.getByText('Skor yorumu')).toBeVisible()
  await expect(page.getByText('Guclu performans')).toBeVisible()
  await expect(page.getByText('Hedef girisi')).toBeVisible()
  await expect(page.getByText('A A - Mukemmel', { exact: true })).toBeVisible()
