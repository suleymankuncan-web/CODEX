...
test('data quality center composes read-only trust signals', async ({ page }) => {
  ...
  await expect(page.getByTestId('admin-metric-workforce')).toContainText('57')
  await expect(page.getByTestId('admin-metric-sourceTrust')).toContainText('0')
  ...
  await expect(main.getByText('Seller-code requests')).toBeVisible()
  await expect(main.getByText('33', { exact: true })).toBeVisible()
  await expect(main.getByText('Offboarding requests', { exact: true })).toBeVisible()
  ...
  await expect(main.getByText('Published KPI config')).toBeVisible()
  await expect(main.getByText('v8', { exact: true })).toBeVisible()
  await expect(main.getByText('Leaderboard source period')).toBeVisible()
  await expect(main.getByText('2026-05-01 - 2026-05-31')).toBeVisible()
