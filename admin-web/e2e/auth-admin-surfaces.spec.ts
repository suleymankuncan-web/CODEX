...
test('auth dashboard page switches chrome to English copy and persists locale', async ({ page }) => {
  ...
  await expect(preview.locator('.stacked-row').filter({ hasText: '/admin/reports' }).getByText('Gorunur')).toBeVisible()
  await expect(preview.locator('.stacked-row').filter({ hasText: '/admin/auth' }).getByText('Kapali')).toBeVisible()
  await expect(
    preview.locator('.stacked-row').filter({ hasText: '/store' }).first().getByText('Gorunur', { exact: true }),
  ).toBeVisible()
  await expect(preview.locator('.stacked-row').filter({ hasText: '/store/reports' }).getByText('Gorunur')).toBeVisible()
  await expect(preview.locator('.stacked-row').filter({ hasText: '/store/feed' }).getByText('Gorunur')).toBeVisible()
  await preview.getByLabel('On izlenecek rol').selectOption('STORE_PERSONNEL')
  await expect(preview.locator('.stacked-row').filter({ hasText: '/store/checklists' }).getByText('Kapali')).toBeVisible()
  ...
  await expect(preview.locator('.stacked-row').filter({ hasText: '/admin/reports' }).getByText('Visible')).toBeVisible()
  await expect(preview.locator('.stacked-row').filter({ hasText: '/admin/auth' }).getByText('Closed')).toBeVisible()
