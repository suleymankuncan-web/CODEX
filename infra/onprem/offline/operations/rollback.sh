...
# Rollback is also a disposable restore rehearsal.  The previous bundle is
# never installed into the active current project and no image-only rollback
# is claimed when migration compatibility is absent.
umask 077
die() { printf '%s\n' "rollback: FAIL: $*" >&2; exit 1; }
say() { printf '%s\n' "rollback: $*"; }
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --previous-bundle-root) [ "$#" -ge 2 ] || die "--previous-bundle-root requires a value"; PREVIOUS_BUNDLE_ROOT=$2; shift 2 ;;
    --release-id) [ "$#" -ge 2 ] || die "--release-id requires a value"; RELEASE_ID=$2; shift 2 ;;
    --previous-release-id) [ "$#" -ge 2 ] || die "--previous-release-id requires a value"; PREVIOUS_RELEASE_ID=$2; shift 2 ;;
    ...
    --help) printf '%s\n' 'usage: rollback.sh --bundle-root CURRENT --previous-bundle-root PREVIOUS --release-id ID --previous-release-id PREVIOUS_ID --source-project PROJECT --target-project PROJECT --public-key PEM --trusted-fingerprint HEX64 --backup-public-key PEM --backup-trusted-fingerprint HEX64 --env-file PATH --backup-dir DIR --receipt PATH --photo-fixture PATH --photo-sha256 HEX64 [--proof-receipt PATH]'; exit 0 ;;
    *) die "unknown argument: $1" ;;
  esac
done
[ -n "$BUNDLE_ROOT" ] && [ -n "$PREVIOUS_BUNDLE_ROOT" ] && [ -n "$RELEASE_ID" ] && [ -n "$PREVIOUS_RELEASE_ID" ] && [ -n "$SOURCE_PROJECT" ] && [ -n "$TARGET_PROJECT" ] && [ -n "$PUBLIC_KEY" ] && [ -n "$TRUSTED_FINGERPRINT" ] && [ -n "$BACKUP_PUBLIC_KEY" ] && [ -n "$BACKUP_TRUSTED_FINGERPRINT" ] && [ -n "$ENV_FILE" ] && [ -n "$BACKUP_DIR" ] && [ -n "$RECEIPT" ] && [ -n "$PHOTO_FIXTURE" ] && [ -n "$PHOTO_SHA256" ] && [ -n "$PHOTO_RECOVERY_HANDLE_FILE" ] || die "current/previous bundles, identities, release/backup trust, env, backup, source, target, receipt, photo fixture, photo hash, and photo recovery handle are required"
[ "$BUNDLE_ROOT" != "$PREVIOUS_BUNDLE_ROOT" ] || die "current and previous bundles must be distinct"
[ "$SOURCE_PROJECT" != "$TARGET_PROJECT" ] || die "rollback target must be distinct from source project"
printf '%s' "$RELEASE_ID" | grep -Eq '^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$' || die "release id is unsafe"
printf '%s' "$PREVIOUS_RELEASE_ID" | grep -Eq '^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$' || die "previous release id is unsafe"
...
if [ -n "$PROOF_COMPOSE" ]; then
  [ "$PROOF_COMPOSE" = "$BUNDLE_ROOT/deployment/proof.compose.yaml" ] || [ "$PROOF_COMPOSE" = "$PREVIOUS_BUNDLE_ROOT/deployment/proof.compose.yaml" ] || die "proof Compose path must be an exact signed deployment/proof.compose.yaml"
...
require_photo_fixture() {
  ...
  case "$PHOTO_FIXTURE" in "$BUNDLE_ROOT"/*|"$PREVIOUS_BUNDLE_ROOT"/*) die "photo fixture must be outside signed bundles" ;; esac
...
require_recovery_handle() {
  ...
  case "$PHOTO_RECOVERY_HANDLE_FILE" in "$BUNDLE_ROOT"/*|"$PREVIOUS_BUNDLE_ROOT"/*) die "photo recovery handle must be outside signed bundles" ;; esac
...
if [ -n "$PROOF_RECEIPT" ]; then
  ...
  case "$PROOF_RECEIPT" in "$BUNDLE_ROOT"|"$BUNDLE_ROOT"/*|"$PREVIOUS_BUNDLE_ROOT"|"$PREVIOUS_BUNDLE_ROOT"/*) die "proof receipt must be outside signed bundles" ;; esac
fi
...
verify_bundle "$PREVIOUS_BUNDLE_ROOT"
...
COMPATIBILITY_OUTPUT=$(node - "$BUNDLE_ROOT/bundle-manifest.json" "$PREVIOUS_BUNDLE_ROOT/bundle-manifest.json" "$COMPATIBILITY" "$RELEASE_ID" "$PREVIOUS_RELEASE_ID" "$BACKUP_DIGEST" <<'NODE'
const fs = require('node:fs')
const [currentPath, previousPath, compatibilityPath, currentRelease, previousRelease, backupDigest] = process.argv.slice(2)
let current, previous, value
try { current = JSON.parse(fs.readFileSync(currentPath, 'utf8')); previous = JSON.parse(fs.readFileSync(previousPath, 'utf8')); value = JSON.parse(fs.readFileSync(compatibilityPath, 'utf8')) } catch { process.exit(41) }
if (!current || !previous || current.releaseId !== currentRelease || previous.releaseId !== previousRelease || !value || value.schemaVersion !== 1 || value.releaseId !== currentRelease || value.rollbackCompatible !== true || typeof value.migrationTreeDigest !== 'string' || !/^[0-9a-f]{64}$/.test(value.migrationTreeDigest) || !Array.isArray(value.compatibleFrom)) process.exit(42)
const entries = value.compatibleFrom.filter((item) => item && item.sourceReleaseId === previousRelease)
if (entries.length !== 1 || backupDigest !== value.migrationTreeDigest) process.exit(43)
const previousDigest = entries[0].sourceMigrationTreeDigest
if (!/^[0-9a-f]{64}$/.test(previousDigest)) process.exit(44)
process.stdout.write(`${value.migrationTreeDigest}|${previousDigest}`)
NODE
) || die "forward_repair_or_database_restore_required"
IFS='|' read -r CURRENT_DIGEST PREVIOUS_DIGEST <<EOF
$COMPATIBILITY_OUTPUT
EOF
printf '%s%s' "$CURRENT_DIGEST" "$PREVIOUS_DIGEST" | grep -Eq '^[0-9a-f]{128}$' || die "forward_repair_or_database_restore_required"

# Prove the disposable rollback target is fresh before the first mutator.
...
TARGET_IMAGE_LINES=$(node - "$PREVIOUS_BUNDLE_ROOT/bundle-manifest.json" <<'NODE'
const fs = require('node:fs')
let manifest
...
if [ -n "$PROOF_COMPOSE" ]; then
  [ "$PROOF_COMPOSE" = "$PREVIOUS_BUNDLE_ROOT/deployment/proof.compose.yaml" ] || die "proof Compose path must match previous signed bundle"
  [ -f "$PROOF_COMPOSE" ] && [ ! -L "$PROOF_COMPOSE" ] || die "previous signed proof Compose overlay is missing or symlinked"
  "$PREVIOUS_BUNDLE_ROOT/operations/preflight.sh" --bundle-root "$PREVIOUS_BUNDLE_ROOT" --release-id "$PREVIOUS_RELEASE_ID" --target-project "$TARGET_PROJECT" --public-key "$PUBLIC_KEY" --trusted-fingerprint "$TRUSTED_FINGERPRINT" --env-file "$TARGET_ENV_FILE" --proof-compose "$PROOF_COMPOSE" --allow-unloaded-images >/dev/null 2>&1 || die "previous target preflight failed"
else
  "$PREVIOUS_BUNDLE_ROOT/operations/preflight.sh" --bundle-root "$PREVIOUS_BUNDLE_ROOT" --release-id "$PREVIOUS_RELEASE_ID" --target-project "$TARGET_PROJECT" --public-key "$PUBLIC_KEY" --trusted-fingerprint "$TRUSTED_FINGERPRINT" --env-file "$TARGET_ENV_FILE" --allow-unloaded-images >/dev/null 2>&1 || die "previous target preflight failed"
fi
revalidate_recovery_handle
revalidate_receipt_parent
IMAGE_SAVE_TEMP_DIR=
