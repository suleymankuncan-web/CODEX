...
RUN cd admin-web \
  && VITE_API_BASE_URL="$VITE_API_BASE_URL" VITE_AUTH_MODE=bearer VITE_AUTH_PROVIDER=oidc VITE_OIDC_AUTO_REDIRECT=true VITE_BROWSER_SESSION_TRANSPORT=cookie VITE_SENTRY_ENABLED=false VITE_SENTRY_DSN= npm run build \
